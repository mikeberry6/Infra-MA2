...
Local verification: ordinary typecheck passed; 38 scoped PortCo/attribution/seed test files passed all 351 tests; offline seed validation had zero errors and 54 existing-format warnings. Regression coverage binds exact scoped changes and preserves source evidence, canonical fields, overlays, redirect baseline and unrelated companies. Initial local test failures were corrected by using the attribution subsystem's own insertion-order hash algorithm and updating the explicit attribution census for the eight removed duplicate records; evidence hashes were not rewritten to hide failures. The first full PR check also identified a stale fund-census test expectation: deleting duplicate raw seed entries reduces its references from 63 to 60 (historical baseline remains 64). Only that expected reference census was updated; the nine deferred fund renames, fund promotion plan, fund data and production links remain unchanged.

After the census expectation correction, the complete local suite passed **162 files / 1,260 tests**. The three retired raw-seed references all concern the existing BCI `Infrastructure & Renewable Resources` label; no fund was renamed or added.
