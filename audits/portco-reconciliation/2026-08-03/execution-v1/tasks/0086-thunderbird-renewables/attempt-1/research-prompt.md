# Thunderbird Renewables — Phase 1 list-reconciliation research
...
- Requested company: **Thunderbird Renewables**
...
- Census holding ID: `012-arclight-capital:holding:017:thunderbird-renewables`
- Census status: `CLOSED_ACTIVE`, high confidence
- Census evidence: <https://arclight.com/investments/>
- Existing production company ID: `cmrxpj6mi00jlivhev0ljria1`
- Existing canonical key: `thunderbird-renewables|united-states`
- Existing evaluated seed key: `thunderbird renewables|United States`
...
- Description claims Fund VIII ownership, SkyVest management, and operating U.S. renewable assets
- Current sources:
...
1. `2024` — Thunderbird Renewables became part of ArcLight's operating-focused renewables initiative.
2. `Jul 2024` — ArcLight launched SkyVest Renewables and disclosed Thunderbird Renewables as one of the initial Fund VIII portfolios it would manage.
...
- Do not conflate Thunderbird Renewables with **SkyVest Renewables**, **Phoenix Renewables**, **Mesquite Sky**, any individual project/subsidiary beneath Thunderbird, or unrelated businesses using Thunderbird in their name.
- Determine whether Thunderbird Renewables is itself a manager-level portfolio/platform record or only an internal portfolio label/passive holding shell that should not be counted independently.
- Do not infer that ArcLight acquired Thunderbird in 2024 merely because SkyVest launched or began managing it in 2024. Preserve `investmentYear: 2024` only if a direct source supports ArcLight's entry/closing in that year; otherwise recommend an exact correction or `NOT_PUBLICLY_DISCLOSED`.
- Do not infer full ownership, a percentage stake, legal-entity suffix, website, founding year, headquarters, or asset capacity.
- ArcLight's 2025 ESG report appears to describe Thunderbird as a SkyVest-managed operating renewables portfolio with **309 MW**. Reopen the report and confirm the exact wording, date scope, units, ownership/management distinction, and whether the number belongs to Thunderbird itself before using it.
...
2. `PROPOSED_CORRECTION` — Thunderbird remains in scope but one or more list-level identity, boundary, geography, active-owner, vehicle, stake, investment-year, status, milestone, description, scale, or primary-citation claims require an exact correction.
3. `SUPERSEDED_OR_DUPLICATE` — Thunderbird is demonstrably another canonical company's alias, predecessor, successor, subsidiary, or duplicate.
...
- Establish Thunderbird Renewables' exact public identity, aliases, legal entity if disclosed, website (if any), headquarters, country, and platform boundary.
- Determine whether it is a distinct manager-level renewable operating portfolio/platform suitable for one PortCo record.
- Reopen ArcLight's current investments and portfolio-services pages and establish whether Thunderbird remains a current Fund VIII investment as of 2026-08-15.
...
- Find any formation or acquisition announcement and legal closing if public. Distinguish an ownership closing from SkyVest's later management-platform launch.
...
- Verify North American qualification, operating-infrastructure scope, and any disclosed portfolio capacity.
...
- Every included fact must have a direct URL whose opened page supports it. One source is sufficient when it directly proves the fact; add evidence when ownership, current status, geography, fund, entry year, scale, or platform boundary is not established by the first.
...
- Regulatory or transaction records found under the exact Thunderbird Renewables name or a directly established legal-entity alias
