...
import {companyImageSha256,verifyProposal,verifyApproval,verifyApplyReceipt} from '../../../../../scripts/portco-reconciliation/artifacts';
import {sha256Canonical} from '../../../../../scripts/portco-reconciliation/hash';
import {prismaCompanyRowToImage} from '../../../../../scripts/portco-reconciliation/prisma-company-image';
import type {ApprovedSeedAfterImageMetadata,SeedRedirectBaselineEntry} from '../../../../../scripts/portco-reconciliation/snapshot';
...
const {projectApprovedSeedOwners}=require('./scripts/portco-reconciliation/approved-seed.ts') as {
 projectApprovedSeedOwners:(image:Parameters<typeof companyImageSha256>[0])=>unknown[];
};
// Runtime-only audited readers keep the seed CLI's enormous evaluated data literal
// outside the application type graph, as in captureSeedIdentity. Validators still run.
const {loadLegacyProductionRedirectLineage,parseApprovedAfterImages,parseSeedRedirectBaseline,assertSeedRedirectBaselineMatchesLiveDecisions}=require('./scripts/portco-reconciliation/snapshot-cli.ts') as {
 loadLegacyProductionRedirectLineage:(root:string,artifact:unknown)=>Promise<Map<string,string[]>>;
 parseApprovedAfterImages:(value:unknown,legacy?:ReadonlyMap<string,readonly string[]>)=>ApprovedSeedAfterImageMetadata[];
 parseSeedRedirectBaseline:(value:unknown)=>SeedRedirectBaselineEntry[];
 assertSeedRedirectBaselineMatchesLiveDecisions:(value:readonly SeedRedirectBaselineEntry[])=>void;
};
...
async function main(){
 ...
 const entries=s.approvedAfterImages,latest=new Map(),entryChecks:{taskId:string;proposalSha256:string;afterImageSha256?:string;projectionExact:boolean;resolvedProjectionExact?:boolean;limitation?:string}[]=[];
 ...
 const metadata:ApprovedSeedAfterImageMetadata[]=entries.flatMap((e:any)=>{try{return parseApprovedAfterImages([e],legacy);}catch(error:any){redirectLineageGaps.push({taskId:e.taskId,proposalSha256:e.proposalSha256,reason:error.message});return [];}});
