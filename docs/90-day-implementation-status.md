...
- Primary-citation packets now include an exact-template-hash-bound semantic worksheet and fail-closed compiler. Exact-equivalent citation rows are grouped without preselection, zero-candidate records remain blocked, and the existing approval schema and complete candidate arrays remain the only apply input.
...
- A protected, default-branch-only Preview dashboard bootstrap now binds an operator-supplied release SHA to exact `main`, exposes the reviewed SHA/Preview tuple before approval, proves the logical Vercel Preview target and health-path protection enforcement, verifies distinct Preview pooled/direct targets and zero schema drift, requires a successful all-provider dry run before live synchronization, and binds the exact persisted `PipelineRun` proof to a freshly authenticated post-write health read. OIDC-capable preflight/postflight jobs never install packages or receive database credentials; the synchronization job has no OIDC authority, registers derived database masks before database access, and uploads evidence only after its final post-manifest credential scan.
...
- The checked-in `preview-bootstrap` workflow is not runnable until it is merged to protected `main` and the protected GitHub environment, Preview-only database/provider secrets, exact host/database variables, and independent reviewer are configured.
...
- Merge the new default-branch Preview bootstrap workflow, configure its protected `preview-bootstrap` environment, and dispatch the exact reviewed `main` SHA so `DASHBOARD_SYNC` can be initialized without exposing production credentials.
