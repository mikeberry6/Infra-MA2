...
- Dashboard/news/import/weekly synchronization instrumentation, public freshness states, scheduled workflows, retry bounds, source/provider thresholds, rolling reliability reports, and retained run artifacts added. News coverage now measures actual upstream page-fetch/search attempts, while dashboard coverage counts only fully successful current provider runs and identifies missing or stale required metrics.
- Publication/source gates, reviewer-neutral citation, ownership-link, and deal seller-disclosure remediation, audited mutation foundations, reviewed duplicate-merge/redirect tooling, and source-coverage reporting added. No migration or release step auto-designates a primary citation, canonical survivor, ownership vehicle, or seller-disclosure treatment.
- Database metrics/rankings, 25-row initial public pages, fixed 25-row URL-addressable admin pagination, shareable URL state, mobile filter sheets, accessible drawers, canonical `/tracker`, grouped search, preview/confirm imports, list/detail payload separation, detail APIs, analytics, and request-correlated structured route/provider timing added. Drawer click-to-committed-shell timing is payload-free and has a 100 ms browser regression budget without waiting for lazy detail data.
...
- The final local repository gate on 2026-07-22 passed locked installation, Prisma validation/generation, lint, typecheck, 117 Vitest files with 740 tests, additive-migration policy, immutable weekly history, offline portfolio/reference validation, latest-weekly-email validation, a zero-vulnerability production dependency audit, and the Node 24 production build. A database-free local Playwright run passed the ten non-database public checks, skipped two credential-dependent authentication checks, and correctly could not complete 24 database-backed journeys; the full browser/axe/visual gate therefore remains unproved until the isolated migrated branch is configured.
- Public database route first-load JavaScript remains below the 150 KB objective in the current build: tracker 129.5 KB, funds 127.8 KB, and portfolio 131.6 KB gzip.
...
- The last fully inspected hosted Draft PR #223 gate before this final continuation, head `d0095e1772ec68bbac21c2612b42c651dfa16ff9`, passed the complete static quality/test/audit/build job and Vercel deployment check in [Release Gate run 29953206620](https://github.com/mikeberry6/Infra-MA2/actions/runs/29953206620). Its historical counts were 108 Vitest files and 656 tests; the final local gate above supersedes those counts, while a fresh hosted run remains required for the new commit.
...
- Complete Research-approved citation decisions, seller-disclosure treatments, ownership-link repairs, duplicate merges with redirects, five current weekly-record reviews, and the `lastVerifiedAt` backlog without rewriting historical weekly editions.
