...
| `DASHBOARD_WRITES_ENABLED` | `true` only after the latest all-source live dry-run artifact has passed review; otherwise unset or `false` |
...
The validation database guard requires `MIGRATION_DATABASE_HOST` and `MIGRATION_DATABASE_NAME` and denies both production endpoints. Before strict source/canonical gates can fail, CI writes reviewer-neutral citation and all-status company-merge templates. If fixed reviewed files exist at `audits/approvals/company-merges.json` and `audits/approvals/primary-citations.json`, CI hashes and applies them only to the isolated validation database, in that order, before strict checks. Reruns are idempotent.

Use two review passes when merges can change citation candidates: commit the reviewed company file first, let validation apply it and regenerate the citation template, then review and commit the post-merge citation file. Do not approve a stale pre-merge citation template. This process allows the gate to become green without weakening it; production still requires the protected manual remediation workflow.
...
Production schema staging, reviewed data remediation, promotion, and rollback are separate workflows serialized by the `production-release` concurrency group and protected by the GitHub `production` environment.
...
- **Stage Production Schema** binds two independently verified baselines: the SHA of the application currently serving the protected canonical production origin and the SHA whose migration names and checksums exactly match production's successfully applied migration ledger. The v2 reviewed manifest binds both identities to the release while diffing the migration baseline to the release; staging requires matching Vercel GitHub source SHA and repository ID, rejects a migration baseline that is not a release ancestor, and repeats both live-app and database-ledger proofs immediately before any write. It then proves both production migration host and database name and applies additive schema while the old application remains live. Immediately after the dashboard review schema lands, it quarantines incompatible prototype USAspending and Federal Register history as `UNAVAILABLE`; then a bounded one-time backfill marks only the prior page's trailing-45-day, active-source, non-sample dashboard signals `APPROVED` and binds them to a SHA-256 content hash. Newly discovered signals always remain `PENDING`. Citation and duplicate outputs are backlog evidence, not implicit approval.
- **Review or Remediate Release Data** separates reports from mutations. Apply operations require a committed `audits/approvals/*.json` artifact, its exact SHA-256, protected-environment approval, exact database targeting, and one explicit operation at a time.
- **Promote Production Release** accepts only a ready staged production deployment supplied by its immutable Vercel URL, whose target is `production`, project ID equals `VERCEL_PROJECT_ID`, GitHub source SHA and repository ID match the release, and health reports its 12-character prefix. It requires clean schema/data/source/pipeline gates, rechecks exact protected-main and candidate provenance immediately before promotion, and promotes the verified deployment ID rather than a mutable alias.

Keep `main` frozen from final manifest review through promotion. If it advances, the previous SHA is ineligible even if it remains an ancestor: regenerate the manifest, wait for the new exact-SHA gate and staged production build, and restage as needed. A Vercel Preview is for pre-merge validation only. With automatic domain assignment disabled, the Git-integrated `main` build is a staged production deployment using production environment values; promoting it changes domains without rebuilding.

## Scheduled pipelines

`.github/workflows/data-pipelines.yml` uses production-target guards, per-pipeline concurrency locks, bounded timeouts, and retries only when output indicates a transient network/provider error.

Dashboard writes are fail-closed. Configure `FRED_API_KEY`, `EIA_API_KEY`, `SAM_API_KEY`, and `SEC_USER_AGENT`, leave `DASHBOARD_WRITES_ENABLED` unset or `false`, and manually dispatch the `source-audit` pipeline. That job runs every dashboard provider in read-only dry-run mode and retains both its log and JSON summary. Operations must confirm that every configured source completed, required metrics are current, representative values agree with the linked official pages, and no credential or source-contract warning remains. Only then may Operations set `DASHBOARD_WRITES_ENABLED=true`; production promotion and scheduled synchronization both reject missing credentials or an unapproved write flag. Revoke the flag while investigating a provider-integrity incident. `SEC_WATCHLIST_CIKS` is optional and does not replace the required SEC user agent.
...
`npm run db:duplicates:verify` separately fails while any normalized duplicate cluster remains in the published public scope. Citation designation and company merging are reviewed remediation operations, never migration backfills. Generate a report, review and commit the exact approval JSON, record its SHA-256, create a restore branch before production writes, apply one decision set, and rerun the strict gates.
...
Logs and artifacts may contain route, operation, duration, status, request ID, public entity IDs, and aggregate counts. They must exclude passwords, emails where avoidable, tokens, cookies, imported row payloads, and private query terms.
