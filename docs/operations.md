...
- **Promote Production Release** accepts only a ready staged production deployment whose Vercel target is `production`, project ID equals `VERCEL_PROJECT_ID`, Git metadata equals the full release SHA, and health reports its 12-character prefix. It requires clean schema/data/source/pipeline gates and rechecks exact protected-main provenance immediately before domain promotion.
