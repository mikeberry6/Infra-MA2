...
- **Stage Production Schema** requires the reviewed base-to-release migration-manifest hash. It rechecks provenance immediately before production writes, proves both production migration host and database name, and applies additive schema while the old application remains live. Legacy empty-hash dashboard signals remain `PENDING` and reviewable; schema staging never auto-approves or auto-publishes them. Citation and duplicate outputs are backlog evidence, not implicit approval.
