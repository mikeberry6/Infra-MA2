...
- Create an isolated Neon validation branch from a recent production snapshot. Configure its direct URL as `MIGRATION_DATABASE_URL`, its exact host as `MIGRATION_DATABASE_HOST`, and its database name as `MIGRATION_DATABASE_NAME`. Never point any of these validation values at production. This snapshot-backed branch is for protected CI validation only; never wire it to Vercel Preview.
