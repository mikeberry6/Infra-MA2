...
async function loadCompanies(publishedOnly = PUBLISHED_ONLY) {
...
async function main() {
  ...
  console.log(`Active canonical rows reviewed: ${companies.length}`);
...
main()
  .catch((error) => {
    reportSafeScriptError("company_merge_report", error);
    process.exitCode = 1;
  })
  .finally(async () => {
