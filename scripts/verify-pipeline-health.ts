...
async function main() {
  ...
  try {
    ...
    const summaryCheck = summaryPath && summaryKind
      ...
      : null;
    const expectedCriticalSources = pipeline === "DASHBOARD_SYNC"
      ? Object.values(DASHBOARD_SOURCES)
        .filter((source) => "critical" in source && source.critical === true)
        .map((source) => ({ id: source.id, name: source.name }))
      : [];
    const completedDashboardWindows = pipeline === "DASHBOARD_SYNC"
      ? refreshWindows
        .filter((window) => window.status !== "RUNNING")
        .map((window) => window.refreshWindow)
      : [];
    const consecutiveSourceIssues = findConsecutiveCriticalSourceIssues(dashboardSourceRuns, {
      expectedSources: expectedCriticalSources,
      refreshWindows: completedDashboardWindows,
    });
    ...
    const report = {
      ...
      evaluatedCriticalSourceWindows: [...new Set(completedDashboardWindows)].sort().slice(-2),
...
main().catch((error) => {
  reportSafeScriptError("pipeline_health", error);
  process.exitCode = 1;
});
