...
import { reportSafeScriptError, SafeOperationalError } from "../src/lib/safe-error.ts";
import {
  coveragePercentage,
...
import { ACTIVE_COMPANY_WHERE } from "../src/modules/companies/retirement.ts";
...
async function main() {
  ...
  try {
    const [
      ...
      prisma.company.count({ where: { status: "PUBLISHED", ...ACTIVE_COMPANY_WHERE } }),
...
main().catch((error) => {
  reportSafeScriptError("source_coverage", error);
  process.exitCode = 1;
});
