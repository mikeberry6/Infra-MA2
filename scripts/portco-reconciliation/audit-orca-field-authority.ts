...
import { proveOrcaFieldAuthority, ORCA, ORCA_SOURCES, ORCA_SOURCE_ROOT, ORCA_PACKET, ORCA_TASK_ROOT } from "./orca-field-authority";
...
const BASE = "68cc7b8cfc09302b03b1fef6f1838f297e69d31d";
...
const PRIOR = "audits/portco-reconciliation/2026-09-06/attribution-field-authority/chief-power/authority.json";
...
const RECEIPT = "audits/portco-reconciliation/2026-08-03/execution-v1/tasks/0079-orca/attempt-1/production-apply/apply-receipt.json";
...
async function main() {
  ...
  const task = execution.tasks.find(row => row.sequence === 79);
  if (!task || task.status !== "COMPLETED" || task.artifacts.proposal?.sha256 !== ORCA.proposalSha256 || !task.artifacts.approval || !task.artifacts.applyReceipt
    || task.artifacts.applyReceipt.location !== RECEIPT
    || task.artifacts.applyReceipt.sha256 !== "7ddac9ef99ce8d1ab509c02b521bf63d6b97f41278514da64c611f18bb74b719") throw new Error("Canonical task79 binding changed");
  if (!task.artifacts.decision || task.artifacts.decision.sha256 !== ORCA.researchSha256
    || sha256Canonical(await json(task.artifacts.decision.location)) !== ORCA.researchSha256
    || task.artifacts.taskSnapshot?.sha256 !== "d671105e6b637ccc92e795bd4d069ee008bb80e5f8895cecc8ef20982407da4a") throw new Error("Canonical research/snapshot binding changed");
  ...
  const dependencies = [CHRONOLOGY, PRIOR, EXECUTION, LEDGER, SEED, FUNDS, "scripts/generate-portfolio-fund-attribution.ts", "scripts/portfolio-fund-attribution/schema.ts",
    ...
    "audits/portco-reconciliation/2026-08-03/execution-v1/tasks/0079-orca/attempt-1/context.json",
    "audits/portco-reconciliation/2026-08-03/execution-v1/tasks/0079-orca/attempt-1/proposal-spec.json",
    "audits/portco-reconciliation/2026-08-03/execution-v1/tasks/0079-orca/attempt-1/task-snapshot.json",
    `${ORCA_TASK_ROOT}/locked-task-snapshot.json`, `${ORCA_TASK_ROOT}/locked-production-snapshot.json`,
    `${ORCA_SOURCE_ROOT}/source-capture.json`,
    ...ORCA_PACKET.map(([file]) => `${ORCA_TASK_ROOT}/${file}`),
  ...
  const sourceCapture = await json(`${ORCA_SOURCE_ROOT}/source-capture.json`);
  if (sourceCapture.sources.length !== 3) throw new Error("Direct source capture cardinality changed");
  for (const source of ORCA_SOURCES.slice(0, 2)) {
    ...
    if (records.length !== 1 || records[0].requestedUrl !== ("url" in source ? source.url : null) || records[0].finalUrl !== ("url" in source ? source.url : null) || records[0].httpStatus !== 200
      || records[0].path !== `${ORCA_SOURCE_ROOT}/${source.file}` || records[0].sha256 !== source.sha256) throw new Error("Direct source capture identity changed");
  }
  for (const source of ORCA_SOURCES) {
    const path = `${ORCA_SOURCE_ROOT}/${source.file}`, bytes = await readFile(path);
    if (!bytes.equals(execFileSync("git", ["show", `${BASE}:${path}`], { maxBuffer: 40_000_000 }))) throw new Error("Protected source bytes changed");
  }
  ...
  try {
    captured = await db.$transaction(async tx => {
      ...
      const image = await loadPrismaCompanyImage(tx, ORCA.companyId);
      const owners = await tx.ownershipPeriod.findMany({ where: { companyId: ORCA.companyId }, select: { id: true, companyId: true, fundId: true,
      ...
      const fund = await tx.fund.findUniqueOrThrow({ where: { id: ORCA.fundId }, select: { id: true, fundName: true, status: true, manager: { select: { name: true } } } });
      const redirects = await tx.companyRedirect.findMany({ where: { OR: [{ companyId: ORCA.companyId }, { retiredId: ORCA.companyId }] }, orderBy: { retiredId: "asc" } });
  ...
  const seedFunds = (await json(FUNDS)).funds.filter((fund: { id: string }) => fund.id === "FUND-153");
  if (seedFunds.length !== 1) throw new Error("Curated seed fund collision");
  const proof = proveOrcaFieldAuthority({ chronology, priorAuthority: await json(PRIOR), seed: await json(SEED), seedFund: seedFunds[0],
    ...
    packet: await Promise.all(ORCA_PACKET.map(async ([file]) => ({ file, bytes: await readFile(`${ORCA_TASK_ROOT}/${file}`) }))),
    sources: await Promise.all(ORCA_SOURCES.map(async source => ({ id: source.id, bytes: await readFile(`${ORCA_SOURCE_ROOT}/${source.file}`) }))) });
