...
export function verifyApprovedSeedText(
  text: string,
  expected: Pick<ApprovedSeedEntry, "proposalSha256" | "approvalSha256" | "afterImageSha256">,
): void {
...
export async function publishApprovedSeedAfterImage(input: {
  ...
  const rendered = renderApprovedSeedArtifact(JSON.parse(currentText), entry);
