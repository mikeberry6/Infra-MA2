...
import { proveArbourFieldAuthority, ARBOUR, ARBOUR_SOURCES, ARBOUR_SOURCE_ROOT, ARBOUR_PACKET, ARBOUR_TASK_ROOT } from "./arbour-field-authority";
...
const BASE = "17a95e01adb2cc01f4cf1cb09ccd4adcbdb38009";
...
const PRIOR = "audits/portco-reconciliation/2026-09-06/attribution-field-authority/chesapeake/authority.json";
...
const SUPERSEDED = "audits/portco-reconciliation/2026-08-03/proposals/0113-arbour-heights-v1/proposal.json";
const ORIGINAL = "audits/portco-reconciliation/2026-09-06/attribution-chronology/production-snapshot.json";
const RECEIPT = "audits/portco-reconciliation/2026-08-03/execution-v1/tasks/0113-arbour-heights/attempt-2/production-apply/apply-receipt.json";
...
async function main() {
  ...
  const task = execution.tasks.find(row => row.sequence === 113);
  if (!task || task.status !== "COMPLETED" || task.artifacts.proposal?.sha256 !== ARBOUR.proposalSha256 || !task.artifacts.approval || !task.artifacts.applyReceipt
    || task.artifacts.applyReceipt.location !== RECEIPT
    || task.artifacts.applyReceipt.sha256 !== "16688769e3217960412e03e6b6861814bbb47eda18f0b76d93a5a0eb1c11a8e7") throw new Error("Canonical task113 binding changed");
  if (!task.artifacts.decision || task.artifacts.decision.sha256 !== "0ed335f43612e203634e16be4af5aae91c1d10e3ea35ef464fa87b4a12d5ea63"
    || sha256Canonical(await json(task.artifacts.decision.location)) !== "0ed335f43612e203634e16be4af5aae91c1d10e3ea35ef464fa87b4a12d5ea63"
    || task.artifacts.taskSnapshot?.sha256 !== "a508d8370f85adbfd4cfa955481c98a6cb57806b1ed25c5bd6b23d8e7cf3c15a") throw new Error("Canonical research/snapshot binding changed");
  ...
  const dependencies = [...new Set([CHRONOLOGY, PRIOR, EXECUTION, LEDGER, SEED, SUPERSEDED, ORIGINAL, "scripts/generate-portfolio-fund-attribution.ts", "scripts/portfolio-fund-attribution/schema.ts",
    ...
    `${ARBOUR_TASK_ROOT}/locked-production-snapshot.json`,
    ...ARBOUR_PACKET.map(([file]) => `${ARBOUR_TASK_ROOT}/${file}`), ...reference.sourceFiles.map((file: { path: string }) => file.path)])];
  ...
  const sourceCapture = await json(`${ARBOUR_SOURCE_ROOT}/source-capture.json`);
  if (sourceCapture.sources.length !== ARBOUR_SOURCES.length) throw new Error("Direct source capture cardinality changed");
  for (const source of ARBOUR_SOURCES) {
    ...
    if (records.length !== 1 || records[0].requestedUrl !== ("url" in source ? source.url : null) || records[0].finalUrl !== ("url" in source ? source.url : null) || records[0].httpStatus !== 200
      || records[0].path !== `${ARBOUR_SOURCE_ROOT}/${source.file}` || records[0].sha256 !== source.sha256) throw new Error("Direct source capture identity changed");
  }
  ...
  try {
    captured = await db.$transaction(async tx => {
      ...
      const image = await loadPrismaCompanyImage(tx, ARBOUR.companyId);
      const owners = await tx.ownershipPeriod.findMany({ where: { companyId: { in: [ARBOUR.companyId, ARBOUR.retiredId] } }, select: { id: true, companyId: true, fundId: true,
      ...
      const redirects = await tx.companyRedirect.findMany({ where: { OR: [{ companyId: ARBOUR.companyId }, { retiredId: ARBOUR.companyId }] }, orderBy: { retiredId: "asc" } });
      const retiredCompany = await tx.company.findUnique({ where: { id: ARBOUR.retiredId }, select: { id: true } });
  ...
  const originalCompanies = original.production.companies.filter((row: {id: string}) => row.id === ARBOUR.companyId);
  if (originalCompanies.length !== 1) throw Error("Original company collision");
  const originalState = { company: originalCompanies[0], redirects: original.production.redirects.filter((row: {companyId: string; retiredId: string}) => row.companyId === ARBOUR.companyId || row.retiredId === ARBOUR.companyId) };
  const proof = proveArbourFieldAuthority({ chronology, priorAuthority: await json(PRIOR), seed: await json(SEED),
    ...
    originalState, correction: await json(task.artifacts.decision.location), supersededProposal: await json(SUPERSEDED),
    seedOverlay: await json("prisma/seed-data/approved-portco-after-images.json"),
    packet: await Promise.all(ARBOUR_PACKET.map(async ([file]) => ({ file, bytes: await readFile(`${ARBOUR_TASK_ROOT}/${file}`) }))),
    sources: await Promise.all(ARBOUR_SOURCES.map(async source => ({ id: source.id, bytes: await readFile(`${ARBOUR_SOURCE_ROOT}/${source.file}`) }))) });
