/** Exclusive raw retrieval of existing task137 attribution sources; no database access. */
...
const historical=JSON.parse(await readFile("audits/portco-reconciliation/2026-08-03/execution-v1/tasks/0137-clearway-energy-group/attempt-1/source-verification.json","utf8"));
const ids=["nrg-signing","tte-signing","company","launch","closing-form3","gip-form4","annual-10k","tte-form4"];
const sources=historical.sources.map((s,i)=>({id:ids[i],url:s.url,file:ids[i]+".html",historicalSha256:s.sha256,historicalHttpStatus:s.httpStatus}));
sources.push({id:"about",url:"https://www.clearwayenergygroup.com/about/",file:"about.html",historicalSha256:null,historicalHttpStatus:null});
sources.push({id:"tte-canonical",url:"https://totalenergies.com/news/press-releases/united-states-totalenergies-acquires-50-clearway-5th-largest-us-renewable",file:"tte-canonical.html",historicalSha256:null,historicalHttpStatus:null});
