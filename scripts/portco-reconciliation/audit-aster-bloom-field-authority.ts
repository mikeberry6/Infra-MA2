...
import { proveAsterFieldAuthority, ASTER, ASTER_SOURCES, ASTER_SOURCE_ROOT, ASTER_PACKET, ASTER_TASK_ROOT, ASTER_ATTRIBUTION_ROOT } from "./aster-bloom-field-authority";
...
const BASE = "1bb355408378fb3efd8432194f8e6e7c7dbb2edd";
...
const PRIOR = "audits/portco-reconciliation/2026-09-06/attribution-field-authority/arbour/authority.json";
...
const RECEIPT = "audits/portco-reconciliation/2026-08-03/execution-v1/tasks/0114-axium-aster-and-axium-bloom/attempt-1/production-apply/apply-receipt.json";
...
async function main() {
  ...
  const task = execution.tasks.find(row => row.sequence === 114);
  if (!task || task.status !== "COMPLETED" || task.artifacts.proposal?.sha256 !== ASTER.proposalSha256 || !task.artifacts.approval || !task.artifacts.applyReceipt
    || task.artifacts.applyReceipt.location !== RECEIPT
    || task.artifacts.applyReceipt.sha256 !== "f68de1961caa67c7aeadb12527bc9ae0b7d39c956cb13544c147dba721467290") throw new Error("Canonical task114 binding changed");
  if (task.artifacts.taskSnapshot?.sha256 !== "6f5dde9733666a0fd627f060e7ba379dbef082cb8ecd59229c9bc899c1ca8831") throw Error("Canonical task snapshot changed");
  const researchBindingPath = `${ASTER_TASK_ROOT}/research-binding.json`;
  if (sha256Canonical(await json(researchBindingPath)) !== ASTER.bindingSha256) throw Error("Research binding changed");
  ...
  const dependencies = [...new Set([CHRONOLOGY, PRIOR, EXECUTION, LEDGER, SEED, ORIGINAL, "scripts/generate-portfolio-fund-attribution.ts", "scripts/portfolio-fund-attribution/schema.ts",
    ...
    `${ASTER_TASK_ROOT}/locked-production-snapshot.json`,
    ...ASTER_PACKET.map(([file]) => `${ASTER_TASK_ROOT}/${file}`), ...reference.sourceFiles.map((file: { path: string }) => file.path),
    `${ASTER_ATTRIBUTION_ROOT}/apply-manifest.json`, `${ASTER_ATTRIBUTION_ROOT}/approval.json`, `${ASTER_ATTRIBUTION_ROOT}/production-apply/apply-receipt.json`])];
  ...
  const sourceCapture = await json(`${ASTER_SOURCE_ROOT}/source-capture.json`);
  ...
  try {
    captured = await db.$transaction(async tx => {
      ...
      const image = await loadPrismaCompanyImage(tx, ASTER.companyId);
      const owners = await tx.ownershipPeriod.findMany({ where: { companyId: ASTER.companyId }, select: { id: true, companyId: true, fundId: true,
      ...
      const redirects = await tx.companyRedirect.findMany({ where: { OR: [{ companyId: ASTER.companyId }, { retiredId: ASTER.companyId }] }, orderBy: { retiredId: "asc" } });
  ...
  const originalCompanies = original.production.companies.filter((row: {id: string}) => row.id === ASTER.companyId);
  if (originalCompanies.length !== 1) throw Error("Original company collision");
  const originalState = { company: originalCompanies[0], redirects: original.production.redirects.filter((row: {companyId: string; retiredId: string}) => row.companyId === ASTER.companyId || row.retiredId === ASTER.companyId) };
  const proof = proveAsterFieldAuthority({ chronology, priorAuthority: await json(PRIOR), seed: await json(SEED),
    ...
    asterAttribution: { manifest: await json(`${ASTER_ATTRIBUTION_ROOT}/apply-manifest.json`), approval: await json(`${ASTER_ATTRIBUTION_ROOT}/approval.json`), receipt: await json(`${ASTER_ATTRIBUTION_ROOT}/production-apply/apply-receipt.json`) },
    seedOverlay: await json("prisma/seed-data/approved-portco-after-images.json"),
    packet: await Promise.all(ASTER_PACKET.map(async ([file]) => ({ file, bytes: await readFile(`${ASTER_TASK_ROOT}/${file}`) }))),
    sources: await Promise.all(ASTER_SOURCES.map(async source => ({ id: source.id, bytes: await readFile(`${ASTER_SOURCE_ROOT}/${source.file}`) }))) });
