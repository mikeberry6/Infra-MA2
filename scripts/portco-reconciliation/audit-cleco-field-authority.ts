...
import { proveClecoFieldAuthority, CLECO, CLECO_SOURCES, CLECO_SOURCE_ROOT, CLECO_PACKET, CLECO_TASK_ROOT, CLECO_BATCH_ROOT } from "./cleco-field-authority";
...
const BASE = "319bc792b967938972572080ec300f1953736271";
...
const PRIOR = "audits/portco-reconciliation/2026-09-07/attribution-field-authority/relam/authority.json";
...
const BATCH_MANIFEST = `${CLECO_BATCH_ROOT}/batch-manifest.json`;
const SEED_SPEC = `${CLECO_BATCH_ROOT}/seed-attribution-reconciliation-spec.json`;
const ORIGINAL = "audits/portco-reconciliation/2026-09-06/attribution-chronology/production-snapshot.json";
const RECEIPT = `${CLECO_BATCH_ROOT}/production-apply/batch-apply-receipt.json`;
...
async function main() {
  ...
  const task = execution.tasks.find(row => row.sequence === 122);
  if (!task || task.status !== "COMPLETED" || task.artifacts.proposal?.sha256 !== CLECO.proposalSha256 || !task.artifacts.approval || !task.artifacts.applyReceipt
    || task.artifacts.applyReceipt.location !== `${RECEIPT}#members/1/receipt`
    || task.artifacts.applyReceipt.sha256 !== CLECO.receiptSha256) throw new Error("Canonical task122 binding changed");
  if (task.artifacts.taskSnapshot?.sha256 !== "e33bb4f55d6fdeba1c688d5440b0f158ab5fcedd3458ca811ace81aeeaa273c2") throw Error("Canonical task snapshot changed");
  ...
  const dependencies = [...new Set([CHRONOLOGY, PRIOR, EXECUTION, LEDGER, SEED, BATCH_MANIFEST, SEED_SPEC, ORIGINAL, "scripts/generate-portfolio-fund-attribution.ts", "scripts/portfolio-fund-attribution/schema.ts",
    ...
    `${CLECO_TASK_ROOT}/attempt-1/locked-production-snapshot.json`,
    CLECO_SOURCES.find(s => s.id === "bci-program")!.path,
    ...CLECO_PACKET.map(([file]) => `${CLECO_TASK_ROOT}/${file}`), ...reference.sourceFiles.map((file: { path: string }) => file.path)])];
  ...
  const sourceCapture = await json(`${CLECO_SOURCE_ROOT}/source-capture.json`);
  ...
  try {
    captured = await db.$transaction(async tx => {
      ...
      const image = await loadPrismaCompanyImage(tx, CLECO.companyId);
      const owners = await tx.ownershipPeriod.findMany({ where: { companyId: CLECO.companyId }, select: { id: true, companyId: true, fundId: true, organizationId: true,
      ...
      const redirects = await tx.companyRedirect.findMany({ where: { OR: [{ companyId: CLECO.companyId }, { retiredId: CLECO.companyId }] }, orderBy: { retiredId: "asc" } });
      const retiredCompany = await tx.company.findUnique({ where: { id: CLECO.retiredId }, select: { id: true } });
  ...
  const originalCompanies = original.production.companies.filter((row: {id: string}) => row.id === CLECO.companyId);
  if (originalCompanies.length !== 1) throw Error("Original company collision");
  const originalState = { company: originalCompanies[0], redirects: original.production.redirects.filter((row: {companyId: string; retiredId: string}) => row.companyId === CLECO.companyId || row.retiredId === CLECO.companyId) };
  const batchReceipt = await json(RECEIPT);
  const proof = proveClecoFieldAuthority({ chronology, priorAuthority: await json(PRIOR), seed: await json(SEED),
    ...
    packet: await Promise.all(CLECO_PACKET.map(async ([file]) => ({ file, bytes: await readFile(`${CLECO_TASK_ROOT}/${file}`) }))),
    sources: await Promise.all(CLECO_SOURCES.map(async source => ({ id: source.id, bytes: await readFile(source.path) }))) });
