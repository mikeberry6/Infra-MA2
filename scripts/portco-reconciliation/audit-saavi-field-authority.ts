...
import { proveSaaviFieldAuthority, SAAVI, SAAVI_SOURCES, SAAVI_SOURCE_ROOT, SAAVI_PACKET, SAAVI_TASK_ROOT, SAAVI_BATCH_ROOT } from "./saavi-field-authority";
...
const BASE = "f13bdb3afafda0943ba18bb46383648ed5d73f0f";
...
const PRIOR = "audits/portco-reconciliation/2026-09-07/attribution-field-authority/revolution/authority.json";
...
const BATCH_MANIFEST = `${SAAVI_BATCH_ROOT}/batch-manifest-v2.json`;
const SEED_SPEC = `${SAAVI_BATCH_ROOT}/seed-attribution-reconciliation-spec.json`;
...
const RECEIPT = `${SAAVI_BATCH_ROOT}/production-apply/batch-apply-receipt.json`;
...
async function main() {
  ...
  const task = execution.tasks.find(row => row.sequence === 152);
  if (!task || task.status !== "COMPLETED" || task.artifacts.proposal?.sha256 !== SAAVI.proposalSha256 || !task.artifacts.approval || !task.artifacts.applyReceipt
    || task.artifacts.applyReceipt.location !== `${RECEIPT}#members/0/receipt`
    || task.artifacts.applyReceipt.sha256 !== SAAVI.receiptSha256) throw new Error("Canonical task152 binding changed");
  if (task.artifacts.taskSnapshot?.sha256 !== "75f6cd5c1a7812e124dd8ee70d0d3eaad0e9900ec540726974ec62d4c554a011") throw Error("Canonical task snapshot changed");
  ...
  const dependencies = [...new Set([CHRONOLOGY, PRIOR, EXECUTION, LEDGER, SEED, BATCH_MANIFEST, SEED_SPEC, SEED_BATCH_MANIFEST, ORIGINAL, "scripts/generate-portfolio-fund-attribution.ts", "scripts/portfolio-fund-attribution/schema.ts",
    ...
    `${SAAVI_TASK_ROOT}/attempt-1/locked-v2-production-snapshot.json`,
    `${SAAVI_TASK_ROOT}/attempt-1/locked-v2-context.json`,
    ...SAAVI_SOURCES.filter(source => source.reused).map(source => source.path),
    ...SAAVI_PACKET.map(([file]) => `${SAAVI_TASK_ROOT}/${file}`), ...reference.sourceFiles.map((file: { path: string }) => file.path), ...laterReference.sourceFiles.map((file:{path:string})=>file.path)])];
  ...
  const sourceCapture = await json(`${SAAVI_SOURCE_ROOT}/source-capture.json`);
  ...
  try {
    captured = await db.$transaction(async tx => {
      ...
      const image = await loadPrismaCompanyImage(tx, SAAVI.companyId);
      const owners = await tx.ownershipPeriod.findMany({ where: { companyId: SAAVI.companyId }, select: { id: true, companyId: true, fundId: true, organizationId: true,
      ...
      const redirects = await tx.companyRedirect.findMany({ where: { OR: [{ companyId: SAAVI.companyId }, { retiredId: SAAVI.companyId }] }, orderBy: { retiredId: "asc" } });
  ...
  const originalCompanies = original.production.companies.filter((row: {id: string}) => row.id === SAAVI.companyId);
  if (originalCompanies.length !== 1) throw Error("Original company collision");
  const originalState = { company: originalCompanies[0], redirects: original.production.redirects.filter((row: {companyId: string; retiredId: string}) => row.companyId === SAAVI.companyId || row.retiredId === SAAVI.companyId) };
  const batchReceipt = await json(RECEIPT);
  const proof = proveSaaviFieldAuthority({ chronology, priorAuthority: await json(PRIOR), seed: await json(SEED),
    ...
    originalState, sourceCapture, filingReview: await json(`${OUT}/filing-review.json`), recoveryCapture: await json(`${OUT}/cofece-curl-capture.json`),
    seedOverlay: await json("prisma/seed-data/approved-portco-after-images.json"),
    packet: await Promise.all(SAAVI_PACKET.map(async ([file]) => ({ file, bytes: await readFile(`${SAAVI_TASK_ROOT}/${file}`) }))),
    sources: await Promise.all(SAAVI_SOURCES.map(async source => ({ id: source.id, bytes: await readFile(source.path) }))) });
