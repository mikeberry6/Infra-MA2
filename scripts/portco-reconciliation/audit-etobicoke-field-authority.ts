...
import { proveEtobicokeFieldAuthority, ETOBICOKE, ETOBICOKE_SOURCES, ETOBICOKE_SOURCE_ROOT, ETOBICOKE_PACKET, ETOBICOKE_TASK_ROOT } from "./etobicoke-field-authority";
...
const BASE = "1b84fef1697a1eb9f1563f03b158364b64bbfcd4";
...
const PRIOR = "audits/portco-reconciliation/2026-09-06/attribution-field-authority/aster-bloom/authority.json";
...
const SUPERSEDED = "audits/portco-reconciliation/2026-08-03/proposals/0117-etobicoke-general-hospital-phase-1-patient-tower-v1/proposal.json";
const ORIGINAL = "audits/portco-reconciliation/2026-09-06/attribution-chronology/production-snapshot.json";
const RECEIPT = "audits/portco-reconciliation/2026-08-03/execution-v1/tasks/0117-etobicoke-general-hospital-phase-1-patient-tower/attempt-2/production-apply/apply-receipt.json";
...
async function main() {
  ...
  const task = execution.tasks.find(row => row.sequence === 117);
  if (!task || task.status !== "COMPLETED" || task.artifacts.proposal?.sha256 !== ETOBICOKE.proposalSha256 || !task.artifacts.approval || !task.artifacts.applyReceipt
    || task.artifacts.applyReceipt.location !== RECEIPT
    || task.artifacts.applyReceipt.sha256 !== "3378304505089234a8f313cc70eecf7aa31016a26b91fd8efe58cf49600be450") throw new Error("Canonical task117 binding changed");
  const researchBindingPath = `${ETOBICOKE_TASK_ROOT}/attempt-2/research-binding.json`;
  if (sha256Canonical(await json(researchBindingPath)) !== ETOBICOKE.bindingSha256
    || task.artifacts.taskSnapshot?.sha256 !== "31703d4ca370777ec3df2f9ea7ec5c45ab41a8ea1b15a2633083cc9e024cdfa3") throw Error("Canonical research/snapshot binding changed");
  const reciprocal = execution.tasks.find(row => row.sequence === 203);
  if (!reciprocal || reciprocal.status !== "SUPERSEDED" || reciprocal.supersededByTaskId !== task.taskId) throw Error("Completed reciprocal boundary changed");
  ...
  const dependencies = [...new Set([CHRONOLOGY, PRIOR, EXECUTION, LEDGER, SEED, SUPERSEDED, ORIGINAL, "scripts/generate-portfolio-fund-attribution.ts", "scripts/portfolio-fund-attribution/schema.ts",
    ...
    researchBindingPath, task.artifacts.taskSnapshot.location,
    `${ETOBICOKE_TASK_ROOT}/attempt-2/locked-production-snapshot.json`,
    ...ETOBICOKE_PACKET.map(([file]) => `${ETOBICOKE_TASK_ROOT}/${file}`), ...reference.sourceFiles.map((file: { path: string }) => file.path)])];
  ...
  const sourceCapture = await json(`${ETOBICOKE_SOURCE_ROOT}/source-capture.json`);
  if (sourceCapture.sources.length !== ETOBICOKE_SOURCES.length) throw new Error("Direct source capture cardinality changed");
  for (const source of ETOBICOKE_SOURCES) {
    ...
    if (records.length !== 1 || records[0].requestedUrl !== ("url" in source ? source.url : null) || records[0].finalUrl !== ("url" in source ? source.url : null) || records[0].httpStatus !== 200
      || records[0].path !== `${ETOBICOKE_SOURCE_ROOT}/${source.file}` || records[0].sha256 !== source.sha256) throw new Error("Direct source capture identity changed");
  }
  ...
  try {
    captured = await db.$transaction(async tx => {
      ...
      const image = await loadPrismaCompanyImage(tx, ETOBICOKE.companyId);
      const owners = await tx.ownershipPeriod.findMany({ where: { companyId: { in: [ETOBICOKE.companyId, ETOBICOKE.retiredId] } }, select: { id: true, companyId: true, fundId: true,
      ...
      const redirects = await tx.companyRedirect.findMany({ where: { OR: [{ companyId: ETOBICOKE.companyId }, { retiredId: ETOBICOKE.companyId }] }, orderBy: { retiredId: "asc" } });
      const retiredCompany = await tx.company.findUnique({ where: { id: ETOBICOKE.retiredId }, select: { id: true } });
  ...
  const originalCompanies = original.production.companies.filter((row: {id: string}) => row.id === ETOBICOKE.companyId);
  if (originalCompanies.length !== 1) throw Error("Original company collision");
  const originalState = { company: originalCompanies[0], redirects: original.production.redirects.filter((row: {companyId: string; retiredId: string}) => row.companyId === ETOBICOKE.companyId || row.retiredId === ETOBICOKE.companyId) };
  const proof = proveEtobicokeFieldAuthority({ chronology, priorAuthority: await json(PRIOR), seed: await json(SEED),
    ...
    packet: await Promise.all(ETOBICOKE_PACKET.map(async ([file]) => ({ file, bytes: await readFile(`${ETOBICOKE_TASK_ROOT}/${file}`) }))),
    sources: await Promise.all(ETOBICOKE_SOURCES.map(async source => ({ id: source.id, bytes: await readFile(`${ETOBICOKE_SOURCE_ROOT}/${source.file}`) }))) });
