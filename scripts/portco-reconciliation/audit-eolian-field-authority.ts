...
import { proveEolianFieldAuthority, EOLIAN, EOLIAN_SOURCES, EOLIAN_SOURCE_ROOT, EOLIAN_PACKET, EOLIAN_TASK_ROOT, EOLIAN_BATCH_ROOT } from "./eolian-field-authority";
...
const BASE = "7d95cc9da5a11b04b6b9cb2d56deb878e4abd08d";
const OUT = "audits/portco-reconciliation/2026-09-07/attribution-field-authority/eolian";
const CHRONOLOGY = "audits/portco-reconciliation/2026-09-06/attribution-chronology/chronology.json";
const PRIOR = "audits/portco-reconciliation/2026-09-07/attribution-field-authority/e360/authority.json";
...
const BATCH_MANIFEST = `${EOLIAN_BATCH_ROOT}/batch-manifest.json`;
...
const RECEIPT = `${EOLIAN_BATCH_ROOT}/production-apply/batch-apply-receipt.json`;
...
async function main() {
  ...
  const task = execution.tasks.find(row => row.sequence === 141);
  if (!task || task.status !== "COMPLETED" || task.artifacts.proposal?.sha256 !== EOLIAN.proposalSha256 || !task.artifacts.approval || !task.artifacts.applyReceipt
    || task.artifacts.applyReceipt.location !== `${RECEIPT}#members/0/receipt`
    || task.artifacts.applyReceipt.sha256 !== EOLIAN.receiptSha256) throw new Error("Canonical task141 binding changed");
  if (task.artifacts.taskSnapshot?.sha256 !== "d808ab5ac8e59af349293ece5fe0a9e3c88c1e860dd9c5e15083085ee392d36a") throw Error("Canonical task snapshot changed");
  ...
  const dependencies = [...new Set([CHRONOLOGY, PRIOR, EXECUTION, LEDGER, SEED, BATCH_MANIFEST, SEED_SPEC, SEED_BATCH_MANIFEST, ORIGINAL, "scripts/generate-portfolio-fund-attribution.ts", "scripts/portfolio-fund-attribution/schema.ts",
    ...
    `${EOLIAN_TASK_ROOT}/attempt-1/locked-production-snapshot.json`,
    `${EOLIAN_TASK_ROOT}/attempt-1/release-delta-check.json`,
    ...EOLIAN_SOURCES.filter(source => source.reused).map(source => source.path),
    ...EOLIAN_PACKET.map(([file]) => `${EOLIAN_TASK_ROOT}/${file}`), ...reference.sourceFiles.map((file: { path: string }) => file.path)])];
  ...
  const sourceCapture = await json(`${EOLIAN_SOURCE_ROOT}/source-capture.json`);
  ...
  try {
    captured = await db.$transaction(async tx => {
      ...
      const image = await loadPrismaCompanyImage(tx, EOLIAN.companyId);
      const owners = await tx.ownershipPeriod.findMany({ where: { companyId: EOLIAN.companyId }, select: { id: true, companyId: true, fundId: true, organizationId: true,
      ...
      const redirects = await tx.companyRedirect.findMany({ where: { OR: [{ companyId: EOLIAN.companyId }, { retiredId: EOLIAN.companyId }] }, orderBy: { retiredId: "asc" } });
  ...
  const originalCompanies = original.production.companies.filter((row: {id: string}) => row.id === EOLIAN.companyId);
  if (originalCompanies.length !== 1) throw Error("Original company collision");
  const originalState = { company: originalCompanies[0], redirects: original.production.redirects.filter((row: {companyId: string; retiredId: string}) => row.companyId === EOLIAN.companyId || row.retiredId === EOLIAN.companyId) };
  const batchReceipt = await json(RECEIPT);
  const proof = proveEolianFieldAuthority({ chronology, priorAuthority: await json(PRIOR), seed: await json(SEED),
    ...
    packet: await Promise.all(EOLIAN_PACKET.map(async ([file]) => ({ file, bytes: await readFile(`${EOLIAN_TASK_ROOT}/${file}`) }))),
    sources: await Promise.all(EOLIAN_SOURCES.map(async source => ({ id: source.id, bytes: await readFile(source.path) }))) });
