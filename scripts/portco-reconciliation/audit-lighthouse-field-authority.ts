...
import { proveLighthouseFieldAuthority, LIGHTHOUSE, LIGHTHOUSE_SOURCES, LIGHTHOUSE_SOURCE_ROOT, LIGHTHOUSE_PACKET, LIGHTHOUSE_TASK_ROOT, LIGHTHOUSE_BATCH_ROOT, LIGHTHOUSE_RENDERED_SOURCES } from "./lighthouse-field-authority";
...
const BASE = "72beca058295653c86a28a5fe65880934b1d9f5d";
const OUT = "audits/portco-reconciliation/2026-09-07/attribution-field-authority/lighthouse";
const CHRONOLOGY = "audits/portco-reconciliation/2026-09-06/attribution-chronology/chronology.json";
const PRIOR = "audits/portco-reconciliation/2026-09-07/attribution-field-authority/eolian/authority.json";
...
const BATCH_MANIFEST = `${LIGHTHOUSE_BATCH_ROOT}/batch-manifest.json`;
const SEED_SPEC = "audits/portco-reconciliation/2026-08-23/batches/batch-0141-0145-v1/seed-attribution-reconciliation-spec.json";
const SEED_BATCH_MANIFEST = "audits/portco-reconciliation/2026-08-23/batches/batch-0141-0145-v1/batch-manifest.json";
const ORIGINAL = "audits/portco-reconciliation/2026-09-06/attribution-chronology/production-snapshot.json";
const RECEIPT = `${LIGHTHOUSE_BATCH_ROOT}/production-apply/batch-apply-receipt.json`;
...
async function main() {
  ...
  const task = execution.tasks.find(row => row.sequence === 142);
  if (!task || task.status !== "COMPLETED" || task.artifacts.proposal?.sha256 !== LIGHTHOUSE.proposalSha256 || !task.artifacts.approval || !task.artifacts.applyReceipt
    || task.artifacts.applyReceipt.location !== `${RECEIPT}#members/1/receipt`
    || task.artifacts.applyReceipt.sha256 !== LIGHTHOUSE.receiptSha256) throw new Error("Canonical task142 binding changed");
  if (task.artifacts.taskSnapshot?.sha256 !== "a755e9b6169b7c373ff8fc5bbf77e76015e986ebf16ac819addfe6bc03d882f3") throw Error("Canonical task snapshot changed");
  ...
  const dependencies = [...new Set([CHRONOLOGY, PRIOR, EXECUTION, LEDGER, SEED, BATCH_MANIFEST, SEED_SPEC, SEED_BATCH_MANIFEST, ORIGINAL, "scripts/generate-portfolio-fund-attribution.ts", "scripts/portfolio-fund-attribution/schema.ts",
    ...
    `${LIGHTHOUSE_TASK_ROOT}/attempt-1/locked-production-snapshot.json`,
    `${LIGHTHOUSE_TASK_ROOT}/attempt-1/release-delta-check.json`,
    ...LIGHTHOUSE_SOURCES.filter(source => source.reused).map(source => source.path),
    ...LIGHTHOUSE_PACKET.map(([file]) => `${LIGHTHOUSE_TASK_ROOT}/${file}`), ...reference.sourceFiles.map((file: { path: string }) => file.path)])];
  ...
  const sourceCapture = await json(`${LIGHTHOUSE_SOURCE_ROOT}/source-capture.json`);
  ...
  try {
    captured = await db.$transaction(async tx => {
      ...
      const image = await loadPrismaCompanyImage(tx, LIGHTHOUSE.companyId);
      const owners = await tx.ownershipPeriod.findMany({ where: { companyId: LIGHTHOUSE.companyId }, select: { id: true, companyId: true, fundId: true, organizationId: true,
      ...
      const redirects = await tx.companyRedirect.findMany({ where: { OR: [{ companyId: LIGHTHOUSE.companyId }, { retiredId: LIGHTHOUSE.companyId }] }, orderBy: { retiredId: "asc" } });
      const retiredCompany = await tx.company.findUnique({ where: { id: LIGHTHOUSE.retiredCompanyId }, select: { id: true } });
  ...
  const originalCompanies = original.production.companies.filter((row: {id: string}) => row.id === LIGHTHOUSE.companyId);
  if (originalCompanies.length !== 1) throw Error("Original company collision");
  const originalState = { company: originalCompanies[0], redirects: original.production.redirects.filter((row: {companyId: string; retiredId: string}) => row.companyId === LIGHTHOUSE.companyId || row.retiredId === LIGHTHOUSE.companyId) };
  const batchReceipt = await json(RECEIPT);
  const proof = proveLighthouseFieldAuthority({ chronology, priorAuthority: await json(PRIOR), seed: await json(SEED),
    proposal: await json(task.artifacts.proposal.location), approval: await json(task.artifacts.approval.location), receipt: batchReceipt.members[1].receipt,
    ...
    originalState, sourceCapture, renderedCapture: await json(`${OUT}/rendered-source-capture.json`),
    renderedSources: await Promise.all(LIGHTHOUSE_RENDERED_SOURCES.map(async source => ({ id: source.id, bytes: await readFile(source.path) }))),
    seedOverlay: await json("prisma/seed-data/approved-portco-after-images.json"),
    packet: await Promise.all(LIGHTHOUSE_PACKET.map(async ([file]) => ({ file, bytes: await readFile(`${LIGHTHOUSE_TASK_ROOT}/${file}`) }))),
    sources: await Promise.all(LIGHTHOUSE_SOURCES.map(async source => ({ id: source.id, bytes: await readFile(source.path) }))) });
