...
import { proveRoverFieldAuthority, ROVER, ROVER_SOURCES, ROVER_SOURCE_ROOT, ROVER_PACKET, ROVER_TASK_ROOT } from "./rover-field-authority";
...
const BASE = "352e6d3360709a96b99528b61f367b2c7d979d94";
...
const PRIOR = "audits/portco-reconciliation/2026-09-06/attribution-field-authority/pike/authority.json";
...
const RECEIPT = "audits/portco-reconciliation/2026-08-03/execution-v1/tasks/0091-rover-pipeline/attempt-2/production-apply/apply-receipt.json";
...
async function main() {
  ...
  const task = execution.tasks.find(row => row.sequence === 91);
  if (!task || task.status !== "COMPLETED" || task.artifacts.proposal?.sha256 !== ROVER.proposalSha256 || !task.artifacts.approval || !task.artifacts.applyReceipt
    || task.artifacts.applyReceipt.location !== RECEIPT
    || task.artifacts.applyReceipt.sha256 !== "67ca39f11c1142085e63c61c26e1948264ab34d160c996460ff6aaf3aade5b20") throw new Error("Canonical task91 binding changed");
  if (!task.artifacts.decision || task.artifacts.decision.sha256 !== "5e7f2754801e53582063a963287ba832c71e6d1f2b4f54175ec6edb80373385c"
    || sha256Canonical(await json(task.artifacts.decision.location)) !== "5e7f2754801e53582063a963287ba832c71e6d1f2b4f54175ec6edb80373385c"
    || task.artifacts.taskSnapshot?.sha256 !== "e23add9f175153cd81fb5fa16522535506e60d7eda6d795e83cbab6eba326741") throw new Error("Canonical research/snapshot binding changed");
  ...
  const dependencies = [CHRONOLOGY, PRIOR, EXECUTION, LEDGER, SEED, FUNDS, "scripts/generate-portfolio-fund-attribution.ts", "scripts/portfolio-fund-attribution/schema.ts",
    ...
    "audits/portco-reconciliation/2026-08-03/execution-v1/tasks/0091-rover-pipeline/attempt-1/context.json",
    "audits/portco-reconciliation/2026-08-03/execution-v1/tasks/0091-rover-pipeline/attempt-1/proposal-spec.json",
    "audits/portco-reconciliation/2026-08-03/execution-v1/tasks/0091-rover-pipeline/attempt-1/task-snapshot.json",
    task.artifacts.decision.location, task.artifacts.taskSnapshot.location,
    "audits/portco-reconciliation/2026-08-03/proposals/0091-rover-pipeline-v1/proposal.json",
    `${ROVER_TASK_ROOT}/locked-task-snapshot.json`, `${ROVER_TASK_ROOT}/locked-production-snapshot.json`,
    ...ROVER_PACKET.map(([file]) => `${ROVER_TASK_ROOT}/${file}`),
  ...
  const sourceCapture = await json(`${ROVER_SOURCE_ROOT}/source-capture.json`);
  if (sourceCapture.sources.length !== 1) throw new Error("Direct source capture cardinality changed");
  for (const source of ROVER_SOURCES) {
    ...
    if (records.length !== 1 || records[0].requestedUrl !== ("url" in source ? source.url : null) || records[0].finalUrl !== ("url" in source ? source.url : null) || records[0].httpStatus !== 200
      || records[0].path !== `${ROVER_SOURCE_ROOT}/${source.file}` || records[0].sha256 !== source.sha256) throw new Error("Direct source capture identity changed");
  }
  ...
  try {
    captured = await db.$transaction(async tx => {
      ...
      const image = await loadPrismaCompanyImage(tx, ROVER.companyId);
      const owners = await tx.ownershipPeriod.findMany({ where: { companyId: ROVER.companyId }, select: { id: true, companyId: true, fundId: true,
      ...
      const fund = await tx.fund.findUniqueOrThrow({ where: { id: ROVER.fundId }, select: { id: true, fundName: true, status: true, manager: { select: { name: true } } } });
      const redirects = await tx.companyRedirect.findMany({ where: { OR: [{ companyId: ROVER.companyId }, { retiredId: ROVER.companyId }] }, orderBy: { retiredId: "asc" } });
  ...
  const seedFunds = (await json(FUNDS)).funds.filter((fund: { id: string }) => fund.id === "FUND-021");
  if (seedFunds.length !== 1) throw new Error("Curated seed fund collision");
  const proof = proveRoverFieldAuthority({ chronology, priorAuthority: await json(PRIOR), seed: await json(SEED), seedFund: seedFunds[0],
    ...
    correction: await json(task.artifacts.decision.location), supersededProposal: await json("audits/portco-reconciliation/2026-08-03/proposals/0091-rover-pipeline-v1/proposal.json"),
    seedOverlay: await json("prisma/seed-data/approved-portco-after-images.json"),
    packet: await Promise.all(ROVER_PACKET.map(async ([file]) => ({ file, bytes: await readFile(`${ROVER_TASK_ROOT}/${file}`) }))),
    sources: await Promise.all(ROVER_SOURCES.map(async source => ({ id: source.id, bytes: await readFile(`${ROVER_SOURCE_ROOT}/${source.file}`) }))) });
