...
import { proveClearwayFieldAuthority, CLEARWAY, CLEARWAY_SOURCES, CLEARWAY_SOURCE_ROOT, CLEARWAY_PACKET, CLEARWAY_TASK_ROOT, CLEARWAY_BATCH_ROOT } from "./clearway-field-authority";
...
const BASE = "c42b0667d19fc9fd4abc3d874bb2d0b6950282d6";
const OUT = "audits/portco-reconciliation/2026-09-07/attribution-field-authority/clearway";
const CHRONOLOGY = "audits/portco-reconciliation/2026-09-06/attribution-chronology/chronology.json";
const PRIOR = "audits/portco-reconciliation/2026-09-07/attribution-field-authority/bluepoint/authority.json";
...
const BATCH_MANIFEST = `${CLEARWAY_BATCH_ROOT}/batch-manifest.json`;
const SEED_SPEC = `${CLEARWAY_BATCH_ROOT}/seed-attribution-reconciliation-spec.json`;
const ORIGINAL = "audits/portco-reconciliation/2026-09-06/attribution-chronology/production-snapshot.json";
const RECEIPT = `${CLEARWAY_BATCH_ROOT}/production-apply/batch-apply-receipt.json`;
...
async function main() {
  ...
  const task = execution.tasks.find(row => row.sequence === 137);
  if (!task || task.status !== "COMPLETED" || task.artifacts.proposal?.sha256 !== CLEARWAY.proposalSha256 || !task.artifacts.approval || !task.artifacts.applyReceipt
    || task.artifacts.applyReceipt.location !== `${RECEIPT}#members/1/receipt`
    || task.artifacts.applyReceipt.sha256 !== CLEARWAY.receiptSha256) throw new Error("Canonical task137 binding changed");
  if (task.artifacts.taskSnapshot?.sha256 !== "8c86dad5c9294d4d68696f09249fb1b4dd569047249587a9a8968cd16706e672") throw Error("Canonical task snapshot changed");
  const chronology = await json(CHRONOLOGY), reference = chronology.receiptReferences.find((row: { pipelineRunId: string }) => row.pipelineRunId === "cmt5lk19w0000duyy1u3zmmno");
  ...
  const dependencies = [...new Set([CHRONOLOGY, PRIOR, EXECUTION, LEDGER, SEED, BATCH_MANIFEST, SEED_SPEC, ORIGINAL, "scripts/generate-portfolio-fund-attribution.ts", "scripts/portfolio-fund-attribution/schema.ts",
    ...
    `${CLEARWAY_TASK_ROOT}/attempt-1/locked-v2-production-snapshot.json`,
    `${CLEARWAY_TASK_ROOT}/attempt-1/release-delta-check.json`,
    ...CLEARWAY_SOURCES.filter(source => source.reused).map(source => source.path),
    ...CLEARWAY_PACKET.map(([file]) => `${CLEARWAY_TASK_ROOT}/${file}`), ...reference.sourceFiles.map((file: { path: string }) => file.path)])];
  ...
  const sourceCapture = await json(`${CLEARWAY_SOURCE_ROOT}/source-capture.json`);
  ...
  try {
    captured = await db.$transaction(async tx => {
      ...
      const image = await loadPrismaCompanyImage(tx, CLEARWAY.companyId);
      const owners = await tx.ownershipPeriod.findMany({ where: { companyId: CLEARWAY.companyId }, select: { id: true, companyId: true, fundId: true, organizationId: true,
      ...
      const redirects = await tx.companyRedirect.findMany({ where: { OR: [{ companyId: CLEARWAY.companyId }, { retiredId: CLEARWAY.companyId }] }, orderBy: { retiredId: "asc" } });
  ...
  const originalCompanies = original.production.companies.filter((row: {id: string}) => row.id === CLEARWAY.companyId);
  if (originalCompanies.length !== 1) throw Error("Original company collision");
  const originalState = { company: originalCompanies[0], redirects: original.production.redirects.filter((row: {companyId: string; retiredId: string}) => row.companyId === CLEARWAY.companyId || row.retiredId === CLEARWAY.companyId) };
  const batchReceipt = await json(RECEIPT);
  const proof = proveClearwayFieldAuthority({ chronology, priorAuthority: await json(PRIOR), seed: await json(SEED),
    ...
    packet: await Promise.all(CLEARWAY_PACKET.map(async ([file]) => ({ file, bytes: await readFile(`${CLEARWAY_TASK_ROOT}/${file}`) }))),
    sources: await Promise.all(CLEARWAY_SOURCES.map(async source => ({ id: source.id, bytes: await readFile(source.path) }))) });
