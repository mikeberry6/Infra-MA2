...
import { proveRelamFieldAuthority, RELAM, RELAM_SOURCES, RELAM_SOURCE_ROOT, RELAM_PACKET, RELAM_TASK_ROOT, RELAM_BATCH_ROOT } from "./relam-field-authority";
...
const BASE = "67c3251854d9bf0cc02c6e7aea3bbb83267efc65";
...
const PRIOR = "audits/portco-reconciliation/2026-09-07/attribution-field-authority/montreal/authority.json";
...
const BATCH_MANIFEST = `${RELAM_BATCH_ROOT}/batch-manifest.json`;
const SEED_SPEC = `${RELAM_BATCH_ROOT}/seed-attribution-reconciliation-spec.json`;
const ORIGINAL = "audits/portco-reconciliation/2026-09-06/attribution-chronology/production-snapshot.json";
const RECEIPT = `${RELAM_BATCH_ROOT}/production-apply/batch-apply-receipt.json`;
...
async function main() {
  ...
  const task = execution.tasks.find(row => row.sequence === 121);
  if (!task || task.status !== "COMPLETED" || task.artifacts.proposal?.sha256 !== RELAM.proposalSha256 || !task.artifacts.approval || !task.artifacts.applyReceipt
    || task.artifacts.applyReceipt.location !== `${RECEIPT}#members/0/receipt`
    || task.artifacts.applyReceipt.sha256 !== RELAM.receiptSha256) throw new Error("Canonical task121 binding changed");
  if (task.artifacts.taskSnapshot?.sha256 !== "6f2b023e18f363ea2e33f44bf37b0b3cb403d186f79d6ea6f737ca5c38773336") throw Error("Canonical task snapshot changed");
  ...
  const dependencies = [...new Set([CHRONOLOGY, PRIOR, EXECUTION, LEDGER, SEED, BATCH_MANIFEST, SEED_SPEC, ORIGINAL, "scripts/generate-portfolio-fund-attribution.ts", "scripts/portfolio-fund-attribution/schema.ts",
    ...
    `${RELAM_TASK_ROOT}/attempt-1/locked-production-snapshot.json`,
    ...RELAM_PACKET.map(([file]) => `${RELAM_TASK_ROOT}/${file}`), ...reference.sourceFiles.map((file: { path: string }) => file.path)])];
  ...
  const sourceCapture = await json(`${RELAM_SOURCE_ROOT}/source-capture.json`);
  const filingCapture = await json(`${RELAM_SOURCE_ROOT}/filing-capture.json`);
  if (sourceCapture.sources.length !== RELAM_SOURCES.length - 1) throw new Error("Direct source capture cardinality changed");
  for (const source of RELAM_SOURCES) {
    const records = source.id === "sec-filing" ? [filingCapture] : sourceCapture.sources.filter((row: { id: string }) => row.id === source.id);
    if (records.length !== 1 || records[0].requestedUrl !== ("url" in source ? source.url : null) || records[0].finalUrl !== ("url" in source ? source.url : null) || records[0].httpStatus !== source.httpStatus
      || records[0].path !== `${RELAM_SOURCE_ROOT}/${source.file}` || records[0].sha256 !== source.sha256 || records[0].bytes !== source.bytes) throw new Error("Direct source capture identity changed");
  }
  ...
  try {
    captured = await db.$transaction(async tx => {
      ...
      const image = await loadPrismaCompanyImage(tx, RELAM.companyId);
      const owners = await tx.ownershipPeriod.findMany({ where: { companyId: RELAM.companyId }, select: { id: true, companyId: true, fundId: true, organizationId: true,
      ...
      const redirects = await tx.companyRedirect.findMany({ where: { OR: [{ companyId: RELAM.companyId }, { retiredId: RELAM.companyId }] }, orderBy: { retiredId: "asc" } });
  ...
  const originalCompanies = original.production.companies.filter((row: {id: string}) => row.id === RELAM.companyId);
  if (originalCompanies.length !== 1) throw Error("Original company collision");
  const originalState = { company: originalCompanies[0], redirects: original.production.redirects.filter((row: {companyId: string; retiredId: string}) => row.companyId === RELAM.companyId || row.retiredId === RELAM.companyId) };
  const batchReceipt = await json(RECEIPT);
  const proof = proveRelamFieldAuthority({ chronology, priorAuthority: await json(PRIOR), seed: await json(SEED),
    ...
    originalState, sourceCapture, filingCapture,
    seedOverlay: await json("prisma/seed-data/approved-portco-after-images.json"),
    packet: await Promise.all(RELAM_PACKET.map(async ([file]) => ({ file, bytes: await readFile(`${RELAM_TASK_ROOT}/${file}`) }))),
    sources: await Promise.all(RELAM_SOURCES.map(async source => ({ id: source.id, bytes: await readFile(`${RELAM_SOURCE_ROOT}/${source.file}`) }))) });
  const report = { ...proof, baseCommit: BASE, executionManifestSha256: execution.manifestSha256, ledgerSha256: ledger.ledgerSha256,
    productionSnapshotSha256: sha256Canonical(production), dependencies: boundFiles, sourceCaptureSha256: sha256Canonical(sourceCapture), filingCaptureSha256: sha256Canonical(filingCapture) };
