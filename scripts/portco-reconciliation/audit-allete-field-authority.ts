...
import { proveAlleteFieldAuthority, ALLETE, ALLETE_SOURCES, ALLETE_SOURCE_ROOT, ALLETE_PACKET, ALLETE_TASK_ROOT, ALLETE_BATCH_ROOT } from "./allete-field-authority";
...
const BASE = "a35c778a40949ebcb429dc50f0734c4d8764e8be";
const OUT = "audits/portco-reconciliation/2026-09-07/attribution-field-authority/allete";
const CHRONOLOGY = "audits/portco-reconciliation/2026-09-06/attribution-chronology/chronology.json";
const PRIOR = "audits/portco-reconciliation/2026-09-07/attribution-field-authority/aligned/authority.json";
...
const BATCH_MANIFEST = `${ALLETE_BATCH_ROOT}/batch-manifest.json`;
const SEED_SPEC = `${ALLETE_BATCH_ROOT}/seed-attribution-reconciliation-spec.json`;
const ORIGINAL = "audits/portco-reconciliation/2026-09-06/attribution-chronology/production-snapshot.json";
const RECEIPT = `${ALLETE_BATCH_ROOT}/production-apply/batch-apply-receipt.json`;
...
async function main() {
  ...
  const task = execution.tasks.find(row => row.sequence === 133);
  if (!task || task.status !== "COMPLETED" || task.artifacts.proposal?.sha256 !== ALLETE.proposalSha256 || !task.artifacts.approval || !task.artifacts.applyReceipt
    || task.artifacts.applyReceipt.location !== `${RECEIPT}#members/2/receipt`
    || task.artifacts.applyReceipt.sha256 !== ALLETE.receiptSha256) throw new Error("Canonical task133 binding changed");
  if (task.artifacts.taskSnapshot?.sha256 !== "7380d7da5c3b6f9ef9723dd104b9733f46506c4ee6d2da74a75cb6b354ccce13") throw Error("Canonical task snapshot changed");
  ...
  const dependencies = [...new Set([CHRONOLOGY, PRIOR, EXECUTION, LEDGER, SEED, BATCH_MANIFEST, SEED_SPEC, ORIGINAL, "scripts/generate-portfolio-fund-attribution.ts", "scripts/portfolio-fund-attribution/schema.ts",
    ...
    `${ALLETE_TASK_ROOT}/attempt-1/locked-production-snapshot.json`,
    `${ALLETE_TASK_ROOT}/attempt-1/release-delta-check.json`,
    ...ALLETE_SOURCES.filter(source => source.reused).map(source => source.path),
    ...ALLETE_PACKET.map(([file]) => `${ALLETE_TASK_ROOT}/${file}`), ...reference.sourceFiles.map((file: { path: string }) => file.path)])];
  ...
  const sourceCapture = await json(`${ALLETE_SOURCE_ROOT}/source-capture.json`);
  ...
  try {
    captured = await db.$transaction(async tx => {
      ...
      const image = await loadPrismaCompanyImage(tx, ALLETE.companyId);
      const owners = await tx.ownershipPeriod.findMany({ where: { companyId: ALLETE.companyId }, select: { id: true, companyId: true, fundId: true, organizationId: true,
      ...
      const redirects = await tx.companyRedirect.findMany({ where: { OR: [{ companyId: ALLETE.companyId }, { retiredId: ALLETE.companyId }] }, orderBy: { retiredId: "asc" } });
  ...
  const originalCompanies = original.production.companies.filter((row: {id: string}) => row.id === ALLETE.companyId);
  if (originalCompanies.length !== 1) throw Error("Original company collision");
  const originalState = { company: originalCompanies[0], redirects: original.production.redirects.filter((row: {companyId: string; retiredId: string}) => row.companyId === ALLETE.companyId || row.retiredId === ALLETE.companyId) };
  const batchReceipt = await json(RECEIPT);
  const proof = proveAlleteFieldAuthority({ chronology, priorAuthority: await json(PRIOR), seed: await json(SEED),
    ...
    packet: await Promise.all(ALLETE_PACKET.map(async ([file]) => ({ file, bytes: await readFile(`${ALLETE_TASK_ROOT}/${file}`) }))),
    sources: await Promise.all(ALLETE_SOURCES.map(async source => ({ id: source.id, bytes: await readFile(source.path) }))) });
