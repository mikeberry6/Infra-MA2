...
import { proveNexusFieldAuthority, NEXUS, NEXUS_SOURCES, NEXUS_SOURCE_ROOT, NEXUS_PACKET, NEXUS_TASK_ROOT, NEXUS_BATCH_ROOT } from "./nexus-field-authority";
...
const BASE = "5dd5c2353c9f687876940a4919b7d1b30294862b";
const OUT = "audits/portco-reconciliation/2026-09-07/attribution-field-authority/nexus";
const CHRONOLOGY = "audits/portco-reconciliation/2026-09-06/attribution-chronology/chronology.json";
const PRIOR = "audits/portco-reconciliation/2026-09-07/attribution-field-authority/mosaic/authority.json";
...
const BATCH_MANIFEST = `${NEXUS_BATCH_ROOT}/batch-manifest.json`;
const SEED_SPEC = `${NEXUS_BATCH_ROOT}/seed-attribution-reconciliation-spec.json`;
const ORIGINAL = "audits/portco-reconciliation/2026-09-06/attribution-chronology/production-snapshot.json";
const RECEIPT = `${NEXUS_BATCH_ROOT}/production-apply/batch-apply-receipt.json`;
...
async function main() {
  ...
  const task = execution.tasks.find(row => row.sequence === 127);
  if (!task || task.status !== "COMPLETED" || task.artifacts.proposal?.sha256 !== NEXUS.proposalSha256 || !task.artifacts.approval || !task.artifacts.applyReceipt
    || task.artifacts.applyReceipt.location !== `${RECEIPT}#members/1/receipt`
    || task.artifacts.applyReceipt.sha256 !== NEXUS.receiptSha256) throw new Error("Canonical task127 binding changed");
  if (task.artifacts.taskSnapshot?.sha256 !== "9bdf96832c514c21d4f4095a43bc30b8e489627b5b8092d7301f7585ee23e263") throw Error("Canonical task snapshot changed");
  ...
  const dependencies = [...new Set([CHRONOLOGY, PRIOR, EXECUTION, LEDGER, SEED, BATCH_MANIFEST, SEED_SPEC, ORIGINAL, "scripts/generate-portfolio-fund-attribution.ts", "scripts/portfolio-fund-attribution/schema.ts",
    ...
    `${NEXUS_TASK_ROOT}/attempt-1/locked-production-snapshot.json`,
    ...NEXUS_SOURCES.filter(source => source.reused).map(source => source.path),
    ...NEXUS_PACKET.map(([file]) => `${NEXUS_TASK_ROOT}/${file}`), ...reference.sourceFiles.map((file: { path: string }) => file.path)])];
  ...
  const sourceCapture = await json(`${NEXUS_SOURCE_ROOT}/source-capture.json`);
  ...
  try {
    captured = await db.$transaction(async tx => {
      ...
      const image = await loadPrismaCompanyImage(tx, NEXUS.companyId);
      const owners = await tx.ownershipPeriod.findMany({ where: { companyId: NEXUS.companyId }, select: { id: true, companyId: true, fundId: true, organizationId: true,
      ...
      const redirects = await tx.companyRedirect.findMany({ where: { OR: [{ companyId: NEXUS.companyId }, { retiredId: NEXUS.companyId }] }, orderBy: { retiredId: "asc" } });
  ...
  const originalCompanies = original.production.companies.filter((row: {id: string}) => row.id === NEXUS.companyId);
  if (originalCompanies.length !== 1) throw Error("Original company collision");
  const originalState = { company: originalCompanies[0], redirects: original.production.redirects.filter((row: {companyId: string; retiredId: string}) => row.companyId === NEXUS.companyId || row.retiredId === NEXUS.companyId) };
  const batchReceipt = await json(RECEIPT);
  const proof = proveNexusFieldAuthority({ chronology, priorAuthority: await json(PRIOR), seed: await json(SEED),
    ...
    packet: await Promise.all(NEXUS_PACKET.map(async ([file]) => ({ file, bytes: await readFile(`${NEXUS_TASK_ROOT}/${file}`) }))),
    sources: await Promise.all(NEXUS_SOURCES.map(async source => ({ id: source.id, bytes: await readFile(source.path) }))) });
