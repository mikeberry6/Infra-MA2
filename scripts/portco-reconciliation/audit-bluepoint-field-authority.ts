...
import { proveBluepointFieldAuthority, BLUEPOINT, BLUEPOINT_SOURCES, BLUEPOINT_SOURCE_ROOT, BLUEPOINT_PACKET, BLUEPOINT_TASK_ROOT, BLUEPOINT_BATCH_ROOT } from "./bluepoint-field-authority";
...
const BASE = "3736b92979d25b33af26b0a2c54aa99012f7db36";
...
const PRIOR = "audits/portco-reconciliation/2026-09-07/attribution-field-authority/allete/authority.json";
...
const BATCH_MANIFEST = `${BLUEPOINT_BATCH_ROOT}/batch-manifest.json`;
const SEED_SPEC = `${BLUEPOINT_BATCH_ROOT}/seed-attribution-reconciliation-spec.json`;
const ORIGINAL = "audits/portco-reconciliation/2026-09-06/attribution-chronology/production-snapshot.json";
const RECEIPT = `${BLUEPOINT_BATCH_ROOT}/production-apply/batch-apply-receipt.json`;
...
async function main() {
  ...
  const task = execution.tasks.find(row => row.sequence === 135);
  if (!task || task.status !== "COMPLETED" || task.artifacts.proposal?.sha256 !== BLUEPOINT.proposalSha256 || !task.artifacts.approval || !task.artifacts.applyReceipt
    || task.artifacts.applyReceipt.location !== `${RECEIPT}#members/4/receipt`
    || task.artifacts.applyReceipt.sha256 !== BLUEPOINT.receiptSha256) throw new Error("Canonical task135 binding changed");
  if (task.artifacts.taskSnapshot?.sha256 !== "af7f94692eb65f9aad0f61195b06f202a4737b1d7d4bfcddf6223f49ea21bfb8") throw Error("Canonical task snapshot changed");
  ...
  const dependencies = [...new Set([CHRONOLOGY, PRIOR, EXECUTION, LEDGER, SEED, BATCH_MANIFEST, SEED_SPEC, ORIGINAL, "scripts/generate-portfolio-fund-attribution.ts", "scripts/portfolio-fund-attribution/schema.ts",
    ...
    `${BLUEPOINT_TASK_ROOT}/attempt-1/locked-production-snapshot.json`,
    `${BLUEPOINT_TASK_ROOT}/attempt-1/release-delta-check.json`,
    ...BLUEPOINT_SOURCES.filter(source => source.reused).map(source => source.path),
    ...BLUEPOINT_PACKET.map(([file]) => `${BLUEPOINT_TASK_ROOT}/${file}`), ...reference.sourceFiles.map((file: { path: string }) => file.path)])];
  ...
  const sourceCapture = await json(`${BLUEPOINT_SOURCE_ROOT}/source-capture.json`);
  ...
  try {
    captured = await db.$transaction(async tx => {
      ...
      const image = await loadPrismaCompanyImage(tx, BLUEPOINT.companyId);
      const owners = await tx.ownershipPeriod.findMany({ where: { companyId: BLUEPOINT.companyId }, select: { id: true, companyId: true, fundId: true, organizationId: true,
      ...
      const redirects = await tx.companyRedirect.findMany({ where: { OR: [{ companyId: BLUEPOINT.companyId }, { retiredId: BLUEPOINT.companyId }] }, orderBy: { retiredId: "asc" } });
  ...
  const originalCompanies = original.production.companies.filter((row: {id: string}) => row.id === BLUEPOINT.companyId);
  if (originalCompanies.length !== 1) throw Error("Original company collision");
  const originalState = { company: originalCompanies[0], redirects: original.production.redirects.filter((row: {companyId: string; retiredId: string}) => row.companyId === BLUEPOINT.companyId || row.retiredId === BLUEPOINT.companyId) };
  const batchReceipt = await json(RECEIPT);
  const proof = proveBluepointFieldAuthority({ chronology, priorAuthority: await json(PRIOR), seed: await json(SEED),
    ...
    packet: await Promise.all(BLUEPOINT_PACKET.map(async ([file]) => ({ file, bytes: await readFile(`${BLUEPOINT_TASK_ROOT}/${file}`) }))),
    sources: await Promise.all(BLUEPOINT_SOURCES.map(async source => ({ id: source.id, bytes: await readFile(source.path) }))) });
