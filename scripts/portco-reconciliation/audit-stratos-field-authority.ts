...
import { proveStratosFieldAuthority, STRATOS, STRATOS_SOURCES, STRATOS_SOURCE_ROOT, STRATOS_PACKET, STRATOS_TASK_ROOT, STRATOS_BATCH_ROOT } from "./stratos-field-authority";
...
const BASE = "23be79d4f3c5f73a73aabe81f470c1eef2e0cd26";
...
const PRIOR = "audits/portco-reconciliation/2026-09-07/attribution-field-authority/saavi/authority.json";
...
const BATCH_MANIFEST = `${STRATOS_BATCH_ROOT}/batch-manifest-v2.json`;
const SEED_SPEC = `${STRATOS_BATCH_ROOT}/seed-attribution-reconciliation-spec.json`;
...
const RECEIPT = `${STRATOS_BATCH_ROOT}/production-apply/batch-apply-receipt.json`;
...
async function main() {
  ...
  const task = execution.tasks.find(row => row.sequence === 153);
  if (!task || task.status !== "COMPLETED" || task.artifacts.proposal?.sha256 !== STRATOS.proposalSha256 || !task.artifacts.approval || !task.artifacts.applyReceipt
    || task.artifacts.applyReceipt.location !== `${RECEIPT}#members/1/receipt`
    || task.artifacts.applyReceipt.sha256 !== STRATOS.receiptSha256) throw new Error("Canonical task153 binding changed");
  if (task.artifacts.taskSnapshot?.sha256 !== "c2082c25100560c9b37ee5f442da2d8331bc0e3664bb27bea3bfbaf946badce1") throw Error("Canonical task snapshot changed");
  ...
  const dependencies = [...new Set([CHRONOLOGY, PRIOR, EXECUTION, LEDGER, SEED, BATCH_MANIFEST, SEED_SPEC, SEED_BATCH_MANIFEST, ORIGINAL, "scripts/generate-portfolio-fund-attribution.ts", "scripts/portfolio-fund-attribution/schema.ts",
    ...
    `${STRATOS_TASK_ROOT}/attempt-1/locked-v2-production-snapshot.json`,
    `${STRATOS_TASK_ROOT}/attempt-1/locked-v2-context.json`,
    ...STRATOS_SOURCES.filter(source => source.reused).map(source => source.path),
    ...STRATOS_PACKET.map(([file]) => `${STRATOS_TASK_ROOT}/${file}`), ...reference.sourceFiles.map((file: { path: string }) => file.path), ...laterReference.sourceFiles.map((file:{path:string})=>file.path)])];
  ...
  const sourceCapture = await json(`${STRATOS_SOURCE_ROOT}/source-capture.json`);
  ...
  try {
    captured = await db.$transaction(async tx => {
      ...
      const image = await loadPrismaCompanyImage(tx, STRATOS.companyId);
      const owners = await tx.ownershipPeriod.findMany({ where: { companyId: STRATOS.companyId }, select: { id: true, companyId: true, fundId: true, organizationId: true,
      ...
      const redirects = await tx.companyRedirect.findMany({ where: { OR: [{ companyId: STRATOS.companyId }, { retiredId: STRATOS.companyId }] }, orderBy: { retiredId: "asc" } });
  ...
  const originalCompanies = original.production.companies.filter((row: {id: string}) => row.id === STRATOS.companyId);
  if (originalCompanies.length !== 1) throw Error("Original company collision");
  const originalState = { company: originalCompanies[0], redirects: original.production.redirects.filter((row: {companyId: string; retiredId: string}) => row.companyId === STRATOS.companyId || row.retiredId === STRATOS.companyId) };
  const batchReceipt = await json(RECEIPT);
  const proof = proveStratosFieldAuthority({ chronology, priorAuthority: await json(PRIOR), seed: await json(SEED),
    ...
    packet: await Promise.all(STRATOS_PACKET.map(async ([file]) => ({ file, bytes: await readFile(`${STRATOS_TASK_ROOT}/${file}`) }))),
    sources: await Promise.all(STRATOS_SOURCES.map(async source => ({ id: source.id, bytes: await readFile(source.path) }))) });
