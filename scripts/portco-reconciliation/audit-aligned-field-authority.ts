...
import { proveAlignedFieldAuthority, ALIGNED, ALIGNED_SOURCES, ALIGNED_SOURCE_ROOT, ALIGNED_PACKET, ALIGNED_TASK_ROOT, ALIGNED_BATCH_ROOT } from "./aligned-field-authority";
...
const BASE = "df2297fde734942072e134e3a032597a2c0e236b";
...
const PRIOR = "audits/portco-reconciliation/2026-09-07/attribution-field-authority/nexus/authority.json";
...
const BATCH_MANIFEST = `${ALIGNED_BATCH_ROOT}/batch-manifest.json`;
const SEED_SPEC = `${ALIGNED_BATCH_ROOT}/seed-attribution-reconciliation-spec.json`;
const ORIGINAL = "audits/portco-reconciliation/2026-09-06/attribution-chronology/production-snapshot.json";
const RECEIPT = `${ALIGNED_BATCH_ROOT}/production-apply/batch-apply-receipt.json`;
...
async function main() {
  ...
  const task = execution.tasks.find(row => row.sequence === 132);
  if (!task || task.status !== "COMPLETED" || task.artifacts.proposal?.sha256 !== ALIGNED.proposalSha256 || !task.artifacts.approval || !task.artifacts.applyReceipt
    || task.artifacts.applyReceipt.location !== `${RECEIPT}#members/1/receipt`
    || task.artifacts.applyReceipt.sha256 !== ALIGNED.receiptSha256) throw new Error("Canonical task132 binding changed");
  if (task.artifacts.taskSnapshot?.sha256 !== "81ab657a750f0651dca25ecdc324d7e1053788dac6d4dfd4782d0a543da65755") throw Error("Canonical task snapshot changed");
  ...
  const dependencies = [...new Set([CHRONOLOGY, PRIOR, EXECUTION, LEDGER, SEED, BATCH_MANIFEST, SEED_SPEC, ORIGINAL, "scripts/generate-portfolio-fund-attribution.ts", "scripts/portfolio-fund-attribution/schema.ts",
    ...
    `${ALIGNED_TASK_ROOT}/attempt-1/locked-v2-production-snapshot.json`,
    `${ALIGNED_TASK_ROOT}/attempt-1/release-delta-check.json`,
    ...ALIGNED_SOURCES.filter(source => source.reused).map(source => source.path),
    ...ALIGNED_PACKET.map(([file]) => `${ALIGNED_TASK_ROOT}/${file}`), ...reference.sourceFiles.map((file: { path: string }) => file.path)])];
  ...
  const sourceCapture = await json(`${ALIGNED_SOURCE_ROOT}/source-capture.json`);
  ...
  try {
    captured = await db.$transaction(async tx => {
      ...
      const image = await loadPrismaCompanyImage(tx, ALIGNED.companyId);
      const owners = await tx.ownershipPeriod.findMany({ where: { companyId: ALIGNED.companyId }, select: { id: true, companyId: true, fundId: true, organizationId: true,
      ...
      const redirects = await tx.companyRedirect.findMany({ where: { OR: [{ companyId: ALIGNED.companyId }, { retiredId: ALIGNED.companyId }] }, orderBy: { retiredId: "asc" } });
  ...
  const originalCompanies = original.production.companies.filter((row: {id: string}) => row.id === ALIGNED.companyId);
  if (originalCompanies.length !== 1) throw Error("Original company collision");
  const originalState = { company: originalCompanies[0], redirects: original.production.redirects.filter((row: {companyId: string; retiredId: string}) => row.companyId === ALIGNED.companyId || row.retiredId === ALIGNED.companyId) };
  const batchReceipt = await json(RECEIPT);
  const proof = proveAlignedFieldAuthority({ chronology, priorAuthority: await json(PRIOR), seed: await json(SEED),
    ...
    packet: await Promise.all(ALIGNED_PACKET.map(async ([file]) => ({ file, bytes: await readFile(`${ALIGNED_TASK_ROOT}/${file}`) }))),
    sources: await Promise.all(ALIGNED_SOURCES.map(async source => ({ id: source.id, bytes: await readFile(source.path) }))) });
