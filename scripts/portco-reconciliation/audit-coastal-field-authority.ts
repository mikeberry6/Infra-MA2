...
import { proveCoastalFieldAuthority, COASTAL_COMPANY, COASTAL_GROUP, COASTAL_PROPOSAL, COASTAL_SOURCES } from "./coastal-field-authority";
...
const BASE = "bef437e6571739da62e7a8573d63404230b40b2b";
...
const PRIOR = "audits/portco-reconciliation/2026-09-06/attribution-field-authority/three-i/authority.json";
...
const TASK = "audits/portco-reconciliation/2026-08-03/execution-v1/tasks/0321-coastal-gaslink-pipeline/attempt-1";
...
async function main() {
  ...
  const batchEntry = ledger.batches.find(row => row.batchId === "batch-0319-0323-v1");
  if (batchEntry?.state !== "COMPLETED" || batchEntry.batchManifest?.path !== `${BATCH}/batch-manifest-v4.json`
    || batchEntry.batchManifest.sha256 !== "9590aadea6ee68e30c519d0160e3c4df5439bc9a3b46dec3942732de5b76fafc") throw new Error("Completed batch reference changed");
  const task = execution.tasks.find(row => row.sequence === 321);
  if (!task || task.status !== "COMPLETED" || task.artifacts.proposal?.sha256 !== COASTAL_PROPOSAL || !task.artifacts.approval
  ...
  const dependencies = [CHRONOLOGY, PRIOR, EXECUTION, LEDGER, SEED, "prisma/seed-data/funds.manifest.json", "scripts/generate-portfolio-fund-attribution.ts",
    ...
    `${TASK}/application-research-decision-v1.json`, `${TASK}/research-decision.json`, `${TASK}/chatgpt-accepted-response.txt`, `${TASK}/source-verification.json`, `${TASK}/source-verification-2026-09-01.json`,
  ...
  try {
    captured = await db.$transaction(async tx => {
      ...
      const image = await loadPrismaCompanyImage(tx, COASTAL_COMPANY);
      const owners = await tx.ownershipPeriod.findMany({ where: { companyId: COASTAL_COMPANY }, select: { id: true, companyId: true, fundId: true,
      ...
      const funds = await tx.fund.findMany({ where: { id: { in: COASTAL_GROUP.flatMap(g => g.fundId ? [g.fundId] : []) } }, select: {
        id: true, fundName: true, status: true, manager: { select: { name: true } } }, orderBy: { id: "asc" } });
      return { image, owners, funds };
    }, { isolationLevel: "RepeatableRead", timeout: 90000 });
  } finally { await db.$disconnect(); }
  const production = JSON.parse(JSON.stringify(captured));
  const proof = proveCoastalFieldAuthority({ chronology, priorAuthority: await json(PRIOR), seed: await json(SEED), seedSpec: await json(`${BATCH}/seed-attribution-reconciliation-spec.json`),
    ...
    attribution: { manifest, approval, receipt }, research: await json(`${TASK}/application-research-decision-v1.json`), production,
    sources: await Promise.all(COASTAL_SOURCES.map(async source => ({ id: source.id, bytes: await readFile(`${OUT}/${source.file}`) }))), filingText: await readFile(`${OUT}/tc-q2-2020-page-35.txt`, "utf8") });
  const report = { ...proof, baseCommit: BASE, executionManifestSha256: execution.manifestSha256, ledgerSha256: ledger.ledgerSha256,
    ...
    visualReview: { oneBasedPage: 35, printedPage: 27, method: "Complete filing page rendered and visually inspected; heading, closing paragraph, named purchaser and retained 35% confirmed." } };
