...
import { proveE360FieldAuthority, E360, E360_SOURCES, E360_SOURCE_ROOT, E360_PACKET, E360_TASK_ROOT, E360_BATCH_ROOT } from "./e360-field-authority";
...
const BASE = "c48cb1abb0d11a0f2d9bd21a5df751781d831703";
const OUT = "audits/portco-reconciliation/2026-09-07/attribution-field-authority/e360";
const CHRONOLOGY = "audits/portco-reconciliation/2026-09-06/attribution-chronology/chronology.json";
const PRIOR = "audits/portco-reconciliation/2026-09-07/attribution-field-authority/clearway/authority.json";
...
const BATCH_MANIFEST = `${E360_BATCH_ROOT}/batch-manifest.json`;
const SEED_SPEC = `${E360_BATCH_ROOT}/seed-attribution-reconciliation-spec.json`;
const ORIGINAL = "audits/portco-reconciliation/2026-09-06/attribution-chronology/production-snapshot.json";
const RECEIPT = `${E360_BATCH_ROOT}/production-apply/batch-apply-receipt.json`;
...
async function main() {
  ...
  const task = execution.tasks.find(row => row.sequence === 140);
  if (!task || task.status !== "COMPLETED" || task.artifacts.proposal?.sha256 !== E360.proposalSha256 || !task.artifacts.approval || !task.artifacts.applyReceipt
    || task.artifacts.applyReceipt.location !== `${RECEIPT}#members/4/receipt`
    || task.artifacts.applyReceipt.sha256 !== E360.receiptSha256) throw new Error("Canonical task140 binding changed");
  if (task.artifacts.taskSnapshot?.sha256 !== "51ec3c273ef3613ab359d71951048b78db90e924f0b7c966ce81ba5ac71894e3") throw Error("Canonical task snapshot changed");
  ...
  const dependencies = [...new Set([CHRONOLOGY, PRIOR, EXECUTION, LEDGER, SEED, BATCH_MANIFEST, SEED_SPEC, ORIGINAL, "scripts/generate-portfolio-fund-attribution.ts", "scripts/portfolio-fund-attribution/schema.ts",
    ...
    `${E360_TASK_ROOT}/attempt-1/locked-v2-production-snapshot.json`,
    `${E360_TASK_ROOT}/attempt-1/release-delta-check.json`,
    ...E360_SOURCES.filter(source => source.reused).map(source => source.path),
    ...E360_PACKET.map(([file]) => `${E360_TASK_ROOT}/${file}`), ...reference.sourceFiles.map((file: { path: string }) => file.path)])];
  ...
  const sourceCapture = await json(`${E360_SOURCE_ROOT}/source-capture.json`);
  ...
  try {
    captured = await db.$transaction(async tx => {
      ...
      const image = await loadPrismaCompanyImage(tx, E360.companyId);
      const owners = await tx.ownershipPeriod.findMany({ where: { companyId: E360.companyId }, select: { id: true, companyId: true, fundId: true, organizationId: true,
      ...
      const redirects = await tx.companyRedirect.findMany({ where: { OR: [{ companyId: E360.companyId }, { retiredId: E360.companyId }] }, orderBy: { retiredId: "asc" } });
  ...
  const originalCompanies = original.production.companies.filter((row: {id: string}) => row.id === E360.companyId);
  if (originalCompanies.length !== 1) throw Error("Original company collision");
  const originalState = { company: originalCompanies[0], redirects: original.production.redirects.filter((row: {companyId: string; retiredId: string}) => row.companyId === E360.companyId || row.retiredId === E360.companyId) };
  const batchReceipt = await json(RECEIPT);
  const proof = proveE360FieldAuthority({ chronology, priorAuthority: await json(PRIOR), seed: await json(SEED),
    proposal: await json(task.artifacts.proposal.location), approval: await json(task.artifacts.approval.location), receipt: batchReceipt.members[4].receipt,
    ...
    packet: await Promise.all(E360_PACKET.map(async ([file]) => ({ file, bytes: await readFile(`${E360_TASK_ROOT}/${file}`) }))),
    sources: await Promise.all(E360_SOURCES.map(async source => ({ id: source.id, bytes: await readFile(source.path) }))) });
