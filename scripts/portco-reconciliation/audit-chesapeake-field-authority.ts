...
import { proveChesapeakeFieldAuthority, CHESAPEAKE, CHESAPEAKE_SOURCES, CHESAPEAKE_SOURCE_ROOT, CHESAPEAKE_PACKET, CHESAPEAKE_TASK_ROOT } from "./chesapeake-field-authority";
...
const BASE = "a54bac1e4024cd5869eb1a1890ba54b014f88ab7";
...
const PRIOR = "audits/portco-reconciliation/2026-09-06/attribution-field-authority/vig/authority.json";
...
const RECEIPT = "audits/portco-reconciliation/2026-08-03/execution-v1/tasks/0112-transurban-chesapeake/attempt-1/production-apply/apply-receipt.json";
...
async function main() {
  ...
  const task = execution.tasks.find(row => row.sequence === 112);
  if (!task || task.status !== "COMPLETED" || task.artifacts.proposal?.sha256 !== CHESAPEAKE.proposalSha256 || !task.artifacts.approval || !task.artifacts.applyReceipt
    || task.artifacts.applyReceipt.location !== RECEIPT
    || task.artifacts.applyReceipt.sha256 !== "a5eb0116ffe7cea43d08ae3c9971e67d05e8c0fc60b77fa743e85bfcc1b6f231") throw new Error("Canonical task112 binding changed");
  if (!task.artifacts.decision || task.artifacts.decision.sha256 !== "2fa86ffee6d4a394b06aceddd3cadbb85cd33abd90f02181fd04820eb0fc4cd2"
    || sha256Canonical(await json(task.artifacts.decision.location)) !== "2fa86ffee6d4a394b06aceddd3cadbb85cd33abd90f02181fd04820eb0fc4cd2"
    || task.artifacts.taskSnapshot?.sha256 !== "f472f85539a78034dbea8b2dc371508c9a507c4dd3366e7152086aa343dd74cb") throw new Error("Canonical research/snapshot binding changed");
  ...
  const dependencies = [...new Set([CHRONOLOGY, PRIOR, EXECUTION, LEDGER, SEED, FUNDS, ORIGINAL, "scripts/generate-portfolio-fund-attribution.ts", "scripts/portfolio-fund-attribution/schema.ts",
    ...
    `${CHESAPEAKE_TASK_ROOT}/locked-production-snapshot.json`,
    ...CHESAPEAKE_PACKET.map(([file]) => `${CHESAPEAKE_TASK_ROOT}/${file}`), ...reference.sourceFiles.map((file: { path: string }) => file.path)])];
  ...
  const sourceCapture = await json(`${CHESAPEAKE_SOURCE_ROOT}/source-capture.json`);
  if (sourceCapture.sources.length !== CHESAPEAKE_SOURCES.length) throw new Error("Direct source capture cardinality changed");
  for (const source of CHESAPEAKE_SOURCES) {
    ...
    if (records.length !== 1 || records[0].requestedUrl !== ("url" in source ? source.url : null) || records[0].finalUrl !== ("url" in source ? source.url : null) || records[0].httpStatus !== 200
      || records[0].path !== `${CHESAPEAKE_SOURCE_ROOT}/${source.file}` || records[0].sha256 !== source.sha256) throw new Error("Direct source capture identity changed");
  }
  ...
  try {
    captured = await db.$transaction(async tx => {
      ...
      const image = await loadPrismaCompanyImage(tx, CHESAPEAKE.companyId);
      const owners = await tx.ownershipPeriod.findMany({ where: { companyId: { in: [CHESAPEAKE.companyId, CHESAPEAKE.retiredId] } }, select: { id: true, companyId: true, fundId: true,
      ...
      const redirects = await tx.companyRedirect.findMany({ where: { OR: [{ companyId: CHESAPEAKE.companyId }, { retiredId: CHESAPEAKE.companyId }] }, orderBy: { retiredId: "asc" } });
      const fund = await tx.fund.findUniqueOrThrow({ where: { id: CHESAPEAKE.fundId }, select: { id: true, fundName: true, status: true, manager: { select: { name: true } } } });
      const retiredCompany = await tx.company.findUnique({ where: { id: CHESAPEAKE.retiredId }, select: { id: true } });
      return { images: [image], owners, fund, redirects, retiredCompany };
  ...
  const originalCompanies = original.production.companies.filter((row: {id: string}) => row.id === CHESAPEAKE.companyId);
  if (originalCompanies.length !== 1) throw Error("Original company collision");
  const originalState = { company: originalCompanies[0], redirects: original.production.redirects.filter((row: {companyId: string; retiredId: string}) => row.companyId === CHESAPEAKE.companyId || row.retiredId === CHESAPEAKE.companyId) };
  const seedFunds = (await json(FUNDS)).funds.filter((row: {id: string}) => row.id === "FUND-026");
  if (seedFunds.length !== 1) throw Error("Seed fund collision");
  const proof = proveChesapeakeFieldAuthority({ chronology, priorAuthority: await json(PRIOR), seed: await json(SEED),
    ...
    seedFund: seedFunds[0], originalState,
    seedOverlay: await json("prisma/seed-data/approved-portco-after-images.json"),
    packet: await Promise.all(CHESAPEAKE_PACKET.map(async ([file]) => ({ file, bytes: await readFile(`${CHESAPEAKE_TASK_ROOT}/${file}`) }))),
    sources: await Promise.all(CHESAPEAKE_SOURCES.map(async source => ({ id: source.id, bytes: await readFile(`${CHESAPEAKE_SOURCE_ROOT}/${source.file}`) }))) });
