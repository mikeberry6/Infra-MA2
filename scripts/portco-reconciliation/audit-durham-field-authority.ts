...
import { proveDurhamFieldAuthority, DURHAM, DURHAM_SOURCES } from "./durham-field-authority";
...
const BASE = "2a2136a78caa0e7275c1e70546902a357ccbadf8";
...
const PRIOR = "audits/portco-reconciliation/2026-09-06/attribution-field-authority/puget-energy/authority.json";
...
async function main() {
  ...
  const task = execution.tasks.find(row => row.sequence === 17);
  if (!task || task.status !== "COMPLETED" || task.artifacts.proposal?.sha256 !== DURHAM.proposalSha256 || !task.artifacts.approval || !task.artifacts.applyReceipt
    || task.artifacts.applyReceipt.sha256 !== "477bc7bed11659c77930627b631c9ff05754c5f3428db6902611a9f431248f01") throw new Error("Canonical task17 binding changed");
  ...
  const dependencies = [CHRONOLOGY, PRIOR, EXECUTION, LEDGER, SEED, FUNDS, "scripts/generate-portfolio-fund-attribution.ts", "scripts/portfolio-fund-attribution/schema.ts",
    task.artifacts.proposal.location, task.artifacts.approval.location, task.artifacts.applyReceipt.location,
    "audits/portco-reconciliation/2026-08-03/execution-v1/tasks/0017-durham-region-courthouse/research-decision.json",
  ...
  try {
    captured = await db.$transaction(async tx => {
      ...
      const image = await loadPrismaCompanyImage(tx, DURHAM.companyId);
      const owners = await tx.ownershipPeriod.findMany({ where: { companyId: DURHAM.companyId }, select: { id: true, companyId: true, fundId: true,
      ...
      const fund = await tx.fund.findUniqueOrThrow({ where: { id: DURHAM.fundId }, select: { id: true, fundName: true, status: true, manager: { select: { name: true } } } });
      const redirects = await tx.companyRedirect.findMany({ where: { OR: [{ companyId: DURHAM.companyId }, { retiredId: DURHAM.companyId }] }, orderBy: { retiredId: "asc" } });
  ...
  const seedFunds = (await json(FUNDS)).funds.filter((fund: { id: string }) => fund.id === "FUND-005");
  if (seedFunds.length !== 1) throw new Error("Curated seed fund collision");
  const proof = proveDurhamFieldAuthority({ chronology, priorAuthority: await json(PRIOR), seed: await json(SEED), seedFund: seedFunds[0],
    ...
    sources: await Promise.all(DURHAM_SOURCES.map(async source => ({ id: source.id, bytes: await readFile(`${OUT}/${source.file}`) }))) });
