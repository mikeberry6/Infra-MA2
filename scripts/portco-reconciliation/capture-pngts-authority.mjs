/** Exclusive raw retrieval of existing task149 canonical/research/seed sources. No database access. */
...
const historical = await json("audits/portco-reconciliation/2026-08-03/execution-v1/tasks/0149-portland-natural-gas-transmission-system/attempt-1/source-verification.json");
const proposal = await json("audits/portco-reconciliation/2026-08-03/proposals/0149-portland-natural-gas-transmission-system-v3/proposal.json");
const chronology = await json("audits/portco-reconciliation/2026-09-06/attribution-chronology/chronology.json");
const ids = ["close", "announcement", "nh-puc", "msip", "profile", "standalone", "ferc"];
if (historical.sources.length !== ids.length) throw Error("Historical source set changed");
const sources = historical.sources.map((s, i) => ({ id: ids[i], url: s.url, file: ids[i] + (i === 2 ? ".response" : ".html"), historicalSha256: s.sha256, historicalHttpStatus: s.httpStatus }));
...
const extraIds = ["msip-legacy", "fitch", "home", "nga"];
