...
import { proveGigapowerFieldAuthority, GIGAPOWER, GIGAPOWER_SOURCES, GIGAPOWER_SOURCE_ROOT, GIGAPOWER_PACKET, GIGAPOWER_TASK_ROOT, GIGAPOWER_BATCH_ROOT } from "./gigapower-field-authority";
...
const BASE = "bb428d24207167be7b5112f20afe8dc5b7d71edb";
...
const PRIOR = "audits/portco-reconciliation/2026-09-07/attribution-field-authority/lighthouse/authority.json";
...
const BATCH_MANIFEST = `${GIGAPOWER_BATCH_ROOT}/batch-manifest.json`;
...
const RECEIPT = `${GIGAPOWER_BATCH_ROOT}/production-apply/batch-apply-receipt.json`;
...
async function main() {
  ...
  const task = execution.tasks.find(row => row.sequence === 143);
  if (!task || task.status !== "COMPLETED" || task.artifacts.proposal?.sha256 !== GIGAPOWER.proposalSha256 || !task.artifacts.approval || !task.artifacts.applyReceipt
    || task.artifacts.applyReceipt.location !== `${RECEIPT}#members/2/receipt`
    || task.artifacts.applyReceipt.sha256 !== GIGAPOWER.receiptSha256) throw new Error("Canonical task143 binding changed");
  if (task.artifacts.taskSnapshot?.sha256 !== "6cf0d0e06e66eec63780827281e7932dad56c346365cbc886e2443dfa1e7321d") throw Error("Canonical task snapshot changed");
  const chronology = await json(CHRONOLOGY), reference = chronology.receiptReferences.find((row: { pipelineRunId: string }) => row.pipelineRunId === "cmt5phzws0000bcyyinnf60dt");
  ...
  const dependencies = [...new Set([CHRONOLOGY, PRIOR, EXECUTION, LEDGER, SEED, BATCH_MANIFEST, SEED_SPEC, SEED_BATCH_MANIFEST, ORIGINAL, "scripts/generate-portfolio-fund-attribution.ts", "scripts/portfolio-fund-attribution/schema.ts",
    ...
    `${GIGAPOWER_TASK_ROOT}/attempt-1/locked-production-snapshot.json`,
    `${GIGAPOWER_TASK_ROOT}/attempt-1/release-delta-check.json`,
    ...GIGAPOWER_SOURCES.filter(source => source.reused).map(source => source.path),
    ...GIGAPOWER_PACKET.map(([file]) => `${GIGAPOWER_TASK_ROOT}/${file}`), ...reference.sourceFiles.map((file: { path: string }) => file.path)])];
  ...
  const sourceCapture = await json(`${GIGAPOWER_SOURCE_ROOT}/source-capture.json`);
  ...
  try {
    captured = await db.$transaction(async tx => {
      ...
      const image = await loadPrismaCompanyImage(tx, GIGAPOWER.companyId);
      const owners = await tx.ownershipPeriod.findMany({ where: { companyId: GIGAPOWER.companyId }, select: { id: true, companyId: true, fundId: true, organizationId: true,
      ...
      const redirects = await tx.companyRedirect.findMany({ where: { OR: [{ companyId: GIGAPOWER.companyId }, { retiredId: GIGAPOWER.companyId }] }, orderBy: { retiredId: "asc" } });
  ...
  const originalCompanies = original.production.companies.filter((row: {id: string}) => row.id === GIGAPOWER.companyId);
  if (originalCompanies.length !== 1) throw Error("Original company collision");
  const originalState = { company: originalCompanies[0], redirects: original.production.redirects.filter((row: {companyId: string; retiredId: string}) => row.companyId === GIGAPOWER.companyId || row.retiredId === GIGAPOWER.companyId) };
  const batchReceipt = await json(RECEIPT);
  const proof = proveGigapowerFieldAuthority({ chronology, priorAuthority: await json(PRIOR), seed: await json(SEED),
    ...
    originalState, sourceCapture, filingText:await readFile(`${OUT}/filing-text.txt`),filingReview:await json(`${OUT}/filing-review.json`),
    seedOverlay: await json("prisma/seed-data/approved-portco-after-images.json"),
    packet: await Promise.all(GIGAPOWER_PACKET.map(async ([file]) => ({ file, bytes: await readFile(`${GIGAPOWER_TASK_ROOT}/${file}`) }))),
    sources: await Promise.all(GIGAPOWER_SOURCES.map(async source => ({ id: source.id, bytes: await readFile(source.path) }))) });
