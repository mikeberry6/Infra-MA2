...
import { provePikeFieldAuthority, PIKE, PIKE_SOURCES, PIKE_SOURCE_ROOT, PIKE_PACKET, PIKE_TASK_ROOT } from "./pike-field-authority";
...
const BASE = "46dcda614f069e4e433a718586fed74c0dbb5d42";
...
const PRIOR = "audits/portco-reconciliation/2026-09-06/attribution-field-authority/orca/authority.json";
...
const RECEIPT = "audits/portco-reconciliation/2026-08-03/execution-v1/tasks/0081-pike-holdings/attempt-1/production-apply/apply-receipt.json";
...
async function main() {
  ...
  const task = execution.tasks.find(row => row.sequence === 81);
  if (!task || task.status !== "COMPLETED" || task.artifacts.proposal?.sha256 !== PIKE.proposalSha256 || !task.artifacts.approval || !task.artifacts.applyReceipt
    || task.artifacts.applyReceipt.location !== RECEIPT
    || task.artifacts.applyReceipt.sha256 !== "063037860b0803792ece2f2219fba2b85b1a2f81f2118ffc0e99c9abd77c4177") throw new Error("Canonical task81 binding changed");
  if (!task.artifacts.decision || task.artifacts.decision.sha256 !== PIKE.researchSha256
    || sha256Canonical(await json(task.artifacts.decision.location)) !== PIKE.researchSha256
    || task.artifacts.taskSnapshot?.sha256 !== "e8f5ac8185a496c6ece8b9ef330026864a8d774fa5babac86d4c819cdd019cb7") throw new Error("Canonical research/snapshot binding changed");
  ...
  const dependencies = [CHRONOLOGY, PRIOR, EXECUTION, LEDGER, SEED, FUNDS, "scripts/generate-portfolio-fund-attribution.ts", "scripts/portfolio-fund-attribution/schema.ts",
    ...
    "audits/portco-reconciliation/2026-08-03/execution-v1/tasks/0081-pike-holdings/attempt-1/context.json",
    "audits/portco-reconciliation/2026-08-03/execution-v1/tasks/0081-pike-holdings/attempt-1/proposal-spec.json",
    "audits/portco-reconciliation/2026-08-03/execution-v1/tasks/0081-pike-holdings/attempt-1/task-snapshot.json",
    `${PIKE_TASK_ROOT}/locked-task-snapshot.json`, `${PIKE_TASK_ROOT}/locked-production-snapshot.json`,
    ...PIKE_PACKET.map(([file]) => `${PIKE_TASK_ROOT}/${file}`),
  ...
  const sourceCapture = await json(`${PIKE_SOURCE_ROOT}/source-capture.json`);
  ...
  for (const source of PIKE_SOURCES) {
    ...
    if (records.length !== 1 || records[0].requestedUrl !== ("url" in source ? source.url : null) || records[0].finalUrl !== ("url" in source ? source.url : null) || records[0].httpStatus !== 200
      || records[0].path !== `${PIKE_SOURCE_ROOT}/${source.file}` || records[0].sha256 !== source.sha256) throw new Error("Direct source capture identity changed");
  }
  ...
  try {
    captured = await db.$transaction(async tx => {
      ...
      const image = await loadPrismaCompanyImage(tx, PIKE.companyId);
      const owners = await tx.ownershipPeriod.findMany({ where: { companyId: PIKE.companyId }, select: { id: true, companyId: true, fundId: true,
      ...
      const fund = await tx.fund.findUniqueOrThrow({ where: { id: PIKE.fundId }, select: { id: true, fundName: true, status: true, manager: { select: { name: true } } } });
      const redirects = await tx.companyRedirect.findMany({ where: { OR: [{ companyId: PIKE.companyId }, { retiredId: PIKE.companyId }] }, orderBy: { retiredId: "asc" } });
  ...
  const proof = provePikeFieldAuthority({ chronology, priorAuthority: await json(PRIOR), seed: await json(SEED), seedFund: seedFunds[0],
    ...
    packet: await Promise.all(PIKE_PACKET.map(async ([file]) => ({ file, bytes: await readFile(`${PIKE_TASK_ROOT}/${file}`) }))),
    sources: await Promise.all(PIKE_SOURCES.map(async source => ({ id: source.id, bytes: await readFile(`${PIKE_SOURCE_ROOT}/${source.file}`) }))) });
