...
import { proveRioGrandeFieldAuthority, RIO_GRANDE_COMPANY, RIO_GRANDE_PROPOSAL, RIO_GRANDE_SOURCES } from "./rio-grande-field-authority";
...
const BASE = "d7a9e90202f14ca91433ee13bfce008cd67c3036";
...
async function main() {
  ...
  const proof = proveRioGrandeFieldAuthority({ chronology, priorAuthority: await json(PRIOR), seed: await json(SEED), seedSpec: await json(`${BATCH}/seed-attribution-reconciliation-spec-v2.json`),
    ...
    sourceVerification: await json(`${TASK}/source-verification.json`), originalFilingText: await readFile(`${OUT}/original-filing.txt`, "utf8"), research: await json(`${TASK}/research-decision.json`), originalSnapshot: await json(ORIGINAL), production,
    sources: await Promise.all(RIO_GRANDE_SOURCES.map(async source => ({ id: source.id, bytes: await readFile(`${OUT}/${source.file}`) }))), filingText: await readFile(`${OUT}/doe.txt`, "utf8") });
  const report = { ...proof, baseCommit: BASE, executionManifestSha256: execution.manifestSha256, ledgerSha256: ledger.ledgerSha256,
    ...
    visualReview: { originalFilingOneBasedPages: [3, 6, 12, 13], doeResponseOneBasedPages: [3, 4], method: "Six complete pages rendered and visually inspected. Original filing: economic versus voting interests, Devonshire/GIC reserve manager, MIC/Mubadala sovereign subsidiary, aggregated project SPV and return thresholds. DOE: GIP-managed T4/T5 partnerships, NextDecade retained corporate equity and Raven/ADNOC affiliate. No diagram percentage substituted for canonical economics." } };
