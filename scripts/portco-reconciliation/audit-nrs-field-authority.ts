...
import { proveNrsFieldAuthority, NRS, NRS_SOURCES, NRS_SOURCE_ROOT, NRS_PACKET, NRS_TASK_ROOT, NRS_BATCH_ROOT } from "./nrs-field-authority";
...
const BASE = "9d2f72a99d6e01ae0581ffed63c8dfcd7de45046";
...
const PRIOR = "audits/portco-reconciliation/2026-09-07/attribution-field-authority/gigapower/authority.json";
...
const BATCH_MANIFEST = `${NRS_BATCH_ROOT}/batch-manifest.json`;
...
const RECEIPT = `${NRS_BATCH_ROOT}/production-apply/batch-apply-receipt.json`;
...
async function main() {
  ...
  const task = execution.tasks.find(row => row.sequence === 148);
  if (!task || task.status !== "COMPLETED" || task.artifacts.proposal?.sha256 !== NRS.proposalSha256 || !task.artifacts.approval || !task.artifacts.applyReceipt
    || task.artifacts.applyReceipt.location !== `${RECEIPT}#members/1/receipt`
    || task.artifacts.applyReceipt.sha256 !== NRS.receiptSha256) throw new Error("Canonical task148 binding changed");
  if (task.artifacts.taskSnapshot?.sha256 !== "bc763a79dd740897eab598225589c03d255a70d70b7dbe4a94c266442423d707") throw Error("Canonical task snapshot changed");
  ...
  const dependencies = [...new Set([CHRONOLOGY, PRIOR, EXECUTION, LEDGER, SEED, BATCH_MANIFEST, SEED_SPEC, SEED_BATCH_MANIFEST, ORIGINAL, "scripts/generate-portfolio-fund-attribution.ts", "scripts/portfolio-fund-attribution/schema.ts",
    ...
    `${NRS_TASK_ROOT}/attempt-1/locked-v2-production-snapshot.json`,
    `${NRS_TASK_ROOT}/attempt-1/locked-v2-context.json`,
    ...NRS_SOURCES.filter(source => source.reused).map(source => source.path),
    ...NRS_PACKET.map(([file]) => `${NRS_TASK_ROOT}/${file}`), ...reference.sourceFiles.map((file: { path: string }) => file.path)])];
  ...
  const sourceCapture = await json(`${NRS_SOURCE_ROOT}/source-capture.json`);
  ...
  try {
    captured = await db.$transaction(async tx => {
      ...
      const image = await loadPrismaCompanyImage(tx, NRS.companyId);
      const owners = await tx.ownershipPeriod.findMany({ where: { companyId: NRS.companyId }, select: { id: true, companyId: true, fundId: true, organizationId: true,
      ...
      const redirects = await tx.companyRedirect.findMany({ where: { OR: [{ companyId: NRS.companyId }, { retiredId: NRS.companyId }] }, orderBy: { retiredId: "asc" } });
  ...
  const originalCompanies = original.production.companies.filter((row: {id: string}) => row.id === NRS.companyId);
  if (originalCompanies.length !== 1) throw Error("Original company collision");
  const originalState = { company: originalCompanies[0], redirects: original.production.redirects.filter((row: {companyId: string; retiredId: string}) => row.companyId === NRS.companyId || row.retiredId === NRS.companyId) };
  const batchReceipt = await json(RECEIPT);
  const proof = proveNrsFieldAuthority({ chronology, priorAuthority: await json(PRIOR), seed: await json(SEED),
    ...
    originalState, sourceCapture, filingReview: await json(`${OUT}/filing-review.json`), filingText: await readFile(`${OUT}/acquisition-text.txt`),
    seedOverlay: await json("prisma/seed-data/approved-portco-after-images.json"),
    packet: await Promise.all(NRS_PACKET.map(async ([file]) => ({ file, bytes: await readFile(`${NRS_TASK_ROOT}/${file}`) }))),
    sources: await Promise.all(NRS_SOURCES.map(async source => ({ id: source.id, bytes: await readFile(source.path) }))) });
  ...
  if (phase === "prepare") {
    ...
    // V2 corrects a narrative-only citation-label claim using the original frozen capture.
    // It does not repeat preparation or change any field decision or evidence bytes.
