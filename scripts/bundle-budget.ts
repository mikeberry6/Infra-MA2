...
export const PUBLIC_JAVASCRIPT_WARNING_BYTES = 135_000;
...
export async function measurePublicRouteBundles({
  ...
  warningBytes = PUBLIC_JAVASCRIPT_WARNING_BYTES,
}: {
  buildDir?: string;
  ...
  if (!Number.isSafeInteger(warningBytes) || warningBytes <= 0) {
    throw new Error("Bundle warning threshold must be a positive integer.");
  }

  return Promise.all(Object.entries(routeBudgets).map(async ([route, budgetBytes]) => {
    if (!Number.isSafeInteger(budgetBytes) || budgetBytes <= 0 || warningBytes > budgetBytes) {
      throw new Error(`Bundle thresholds for ${route} are invalid.`);
    }
    ...
    const status = gzipBytes > budgetBytes
      ? "failed"
      : gzipBytes > warningBytes ? "warning" : "ok";
    return {
      route,
      budgetBytes,
      warningBytes,
