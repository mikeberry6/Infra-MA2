...
describe("portfolio fund attribution seed manifest", () => {
  it("matches every active evaluated seed owner exactly once", () => {
    ...
    expect(manifest.records).toHaveLength(1_403);
    expect(owners).toHaveLength(1_403);
  ...
  it("does not create funds and labels every estimate", () => {
    ...
    expect(manifest.policy.inferredAssignments).toBe(463);
    expect(inferred).toHaveLength(463);
