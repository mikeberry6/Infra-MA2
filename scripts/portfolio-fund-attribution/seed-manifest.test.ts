...
describe("portfolio fund attribution seed manifest", () => {
  it("matches every active evaluated seed owner exactly once", () => {
    ...
    expect(manifest.records).toHaveLength(1_311);
    expect(owners).toHaveLength(1_311);
  ...
  it("does not create funds and labels every estimate", () => {
    ...
    expect(manifest.policy.inferredAssignments).toBe(531);
    expect(inferred).toHaveLength(531);
    expect(manifest.records.filter((record) => record.fundAttribution === "UNRESOLVED")).toHaveLength(30);
