...
describe("portfolio fund attribution seed manifest", () => {
  ...
  it("does not create funds and labels every estimate", () => {
    ...
    expect(manifest.policy.inferredAssignments).toBe(468);
    expect(inferred).toHaveLength(468);
    expect(manifest.records.filter((record) => record.fundAttribution === "UNRESOLVED")).toHaveLength(67);
