...
describe("portfolio fund attribution seed manifest", () => {
  ...
  it("does not create funds and labels every estimate", () => {
    ...
    expect(manifest.policy.inferredAssignments).toBe(481);
    expect(inferred).toHaveLength(481);
    expect(manifest.records.filter((record) => record.fundAttribution === "UNRESOLVED")).toHaveLength(57);
