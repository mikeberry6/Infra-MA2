...
describe("portfolio fund attribution seed manifest", () => {
  it("matches every active evaluated seed owner exactly once", () => {
    ...
    expect(manifest.records).toHaveLength(1_287);
    expect(owners).toHaveLength(1_287);
  ...
  it("does not create funds and labels every estimate", () => {
    ...
    expect(manifest.policy.inferredAssignments).toBe(579);
    expect(inferred).toHaveLength(579);
