...
describe("portfolio fund attribution seed manifest", () => {
  it("matches every active evaluated seed owner exactly once", () => {
    ...
    expect(manifest.records).toHaveLength(1_391);
    expect(owners).toHaveLength(1_391);
  ...
  it("does not create funds and labels every estimate", () => {
    ...
    expect(manifest.records.filter((record) => record.fundAttribution === "UNRESOLVED")).toHaveLength(55);
