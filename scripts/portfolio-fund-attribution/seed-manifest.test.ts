...
describe("portfolio fund attribution seed manifest", () => {
  it("matches every active evaluated seed owner exactly once", () => {
    ...
    expect(manifest.records).toHaveLength(1_282);
    expect(owners).toHaveLength(1_282);
  ...
  it("does not create funds and labels every estimate", () => {
    ...
    expect(manifest.policy.inferredAssignments).toBe(544);
    expect(inferred).toHaveLength(544);
    expect(manifest.records.filter((record) => record.fundAttribution === "UNRESOLVED")).toHaveLength(22);
