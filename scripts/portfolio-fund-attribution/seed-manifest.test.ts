...
describe("portfolio fund attribution seed manifest", () => {
  it("matches every active evaluated seed owner exactly once", () => {
    ...
    // All 1,393 active owners remain covered. Batches 019–021 add fifteen proven former-owner
    // metadata overlays, not owners; every added key must exist in evaluated seed.
    expect(manifest.records).toHaveLength(1_408);
  ...
  it("does not create funds and labels every estimate", () => {
    ...
    // Batch 021 preserves exact unknown funds, including former-owner seed-only annotations.
    expect(manifest.records.filter((record) => record.fundAttribution === "UNRESOLVED")).toHaveLength(84);
