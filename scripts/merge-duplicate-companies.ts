 ...
 * never silently chooses between materially different child records and does
 * not delete retired Company rows while the Phase 1 rollback remains viable.
 */
...
import {
  ...
  type RetiredCompatibilityEvidence,
...
} from "../src/modules/companies/retirement";
...
interface ApplyTotals {
  updated: number;
  unchanged: number;
  compatibilityRowsPreserved: number;
...
function retiredCompatibilityEvidence(
  value: Prisma.JsonValue | undefined,
): RetiredCompatibilityEvidence[] | null {
  if (!Array.isArray(value)) return null;
  const parsed: RetiredCompatibilityEvidence[] = [];
  for (const item of value) {
    const object = jsonObject(item);
    if (typeof object?.id !== "string"
      || typeof object.compatibilityIdentitySha256 !== "string"
      || !/^[0-9a-f]{64}$/.test(object.compatibilityIdentitySha256)) {
      return null;
    }
    parsed.push({
      id: object.id,
      compatibilityIdentitySha256: object.compatibilityIdentitySha256,
    });
  }
  return parsed;
}
...
async function assertExactReplay(
  ...
  const exactAudit = audits.find((audit) => {
  ...
  const exactChanges = jsonObject(exactAudit.changes);
  const evidence = retiredCompatibilityEvidence(exactChanges?.retiredCompatibilityIdentities);
  if (!evidence) {
    throw new Error(`Approval replay for ${cluster.canonicalId} has no exact retired compatibility evidence`);
  }
  await assertNoRetiredRelations(tx, cluster.retiredIds);
  const tombstones = await tx.company.findMany({
    where: { id: { in: cluster.retiredIds } },
...
async function assertNoRetiredRelations(
  ...
  if (ownershipPeriods + milestones + managementRoles + citations + newsMentions !== 0) {
    throw new Error("A retired company still owns relations; refusing canonical retirement");
  }
}
...
async function applyPendingCluster(input: {
...
}): Promise<{ compatibilityRowsPreserved: number; redirectsRehomed: number; stats: ReturnType<typeof relationStats> }> {
  ...
  const retiredCompatibilityIdentities = retired.map((company) => ({
    id: company.id,
    compatibilityIdentitySha256: companyCompatibilityIdentitySha256(company),
  }));
  ...
  if (remainingRetired !== approved.retiredIds.length
  ...
  await tx.auditEvent.create({
    data: {
      ...
      changes: {
        ...
        compatibilityRowsPreserved: approved.retiredIds.length,
        retiredCompatibilityIdentities,
      },
      metadata: {
...
async function applyReviewedApproval(
  ...
  const [companies, redirects] = await Promise.all([
    tx.company.findMany({ select: COMPANY_MERGE_SNAPSHOT_SELECT }),
    tx.companyRedirect.findMany({ select: { retiredId: true, companyId: true } }),
  ]);
  const byId = new Map(companies.map((company) => [company.id, company]));
  const retirements = companyRetirementMap(redirects);
  const detected = duplicateClusters(excludeRedirectedCompanies(companies, redirects));
  const totals: ApplyTotals = {
    updated: 0,
    unchanged: 0,
    compatibilityRowsPreserved: 0,
    redirectsRehomed: 0,
  ...
  for (const approved of approval.clusters) {
    ...
    if (retirements.has(approved.canonicalId)) {
      throw new Error(`Approved canonical company ${approved.canonicalId} is already retired`);
    }
    const conflictingRetirements = approved.retiredIds.filter((id) => {
      const survivor = retirements.get(id);
      return survivor !== undefined && survivor !== approved.canonicalId;
    });
    if (conflictingRetirements.length > 0) {
      throw new Error(`Approved retired IDs point to a different canonical company: ${conflictingRetirements.join(", ")}`);
    }
    const appliedRetirements = approved.retiredIds.filter(
      (id) => retirements.get(id) === approved.canonicalId,
    );
    if (appliedRetirements.length === approved.retiredIds.length) {
    ...
    if (appliedRetirements.length > 0) {
    ...
    if (liveRetired.length !== approved.retiredIds.length) {
      throw new Error(`The reviewed retired-company set changed before apply`);
    }
  ...
  for (const item of pending) {
    ...
    totals.updated += 1;
    totals.compatibilityRowsPreserved += result.compatibilityRowsPreserved;
...
async function main() {
  ...
  try {
    if (!APPLY) {
      const [companies, redirects] = await Promise.all([
        prisma.company.findMany({ select: COMPANY_MERGE_SNAPSHOT_SELECT }),
        prisma.companyRedirect.findMany({ select: { retiredId: true } }),
      ]);
      const clusters = duplicateClusters(excludeRedirectedCompanies(companies, redirects));
...
main().catch((error) => {
  reportSafeScriptError("company_merge", error);
  process.exitCode = 1;
});
