...
} from "@/modules/companies/retirement";
import {
  companyIdentityConflictMessage,
  queryCompanyIdentityConflicts,
  ...
  try {
    ...
    const deal = await prisma.$transaction(async (tx) => {
      ...
      await auditMutation("Deal", deal.id, "CREATE", [
        "legacyId", "title", "target", "sector", "subsector", "region", "categories",
        "date", "description", "targetDescription", "country", "dealStatus",
        "enterpriseValue", "equityValue", "stake", "closingDate", "assetScale",
        "valuationMultiple", "fundVehicle", "keyHighlights", "sellerDisclosureStatus",
        "sellerDisclosureReason", "status", "participants", "citations",
      ], tx);
      return deal;
    });
...
export async function updateDeal(id: string, formData: FormData): Promise<ActionResult> {
  try {
    ...
    const dealUpdateData = {
      ...
      sellerDisclosureStatus: finalSellers.length > 0 ? "DISCLOSED" as const : d.sellerDisclosureStatus,
      sellerDisclosureReason: finalSellers.length > 0 ? null : d.sellerDisclosureReason?.trim() || null,
    ...
    const changedFields = changedFieldSummary(
      {
        ...
        buyers: existingDeal.participants.filter((item) => item.role === "BUYER").map((item) => item.displayName).sort(),
        sellers: existingDeal.participants.filter((item) => item.role === "SELLER").map((item) => item.displayName).sort(),
        primarySourceUrl: existingDeal.citations[0]?.source.url ?? null,
      },
      {
        ...dealUpdateData,
    ...
    await prisma.$transaction(async (tx) => {
      const updateResult = await tx.deal.updateMany({
        where: { id, status: existingDeal.status, updatedAt: existingDeal.updatedAt },
        data: dealUpdateData,
      });
  ...
  try {
    ...
    const fund = await prisma.$transaction(async (tx) => {
      ...
      await auditMutation("Fund", fund.id, "CREATE", [
        "legacyId", "managerId", "fundName", "ticker", "investmentStrategy", "sourceUrls",
        "primarySourceUrl", "size", "sizeUsdMm", "vintage", "strategies", "structure",
        "fundStatus", "sectors", "regions", "strategyUrl", "status",
      ], tx);
      return fund;
    });
  ...
  try {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      const updateResult = await tx.fund.updateMany({
        where: { id, status: existingFund.status, updatedAt: existingFund.updatedAt },
        data: {
...
export async function createCompany(formData: FormData): Promise<ActionResult> {
  try {
    ...
    const company = await prisma.$transaction(async (tx) => {
      const identityConflicts = await queryCompanyIdentityConflicts(tx, c.name);
      if (identityConflicts.length > 0) {
        throw new AdminActionUserError(companyIdentityConflictMessage(identityConflicts));
      }
      ...
      await auditMutation("Company", company.id, "CREATE", [
        "name", "country", "sector", "subsector", "region", "description", "companyStatus",
        "website", "yearFounded", "headquarters", "countryTags", "status",
        "ownershipPeriods", "citations",
      ], tx);
      return company;
    }, { isolationLevel: "Serializable" });
    ...
    const changedFields = changedFieldSummary(
      {
        ...
        primarySourceUrl: existingCompany.citations[0]?.source.url ?? null,
      },
      { ...companyUpdateData, primarySourceUrl: c.sourceUrl || null },
    );
    if (changedFields.length === 0) changedFields.push("updatedAt");

    await prisma.$transaction(async (tx) => {
      const identityConflicts = await queryCompanyIdentityConflicts(tx, c.name, id);
      if (identityConflicts.length > 0) {
        throw new AdminActionUserError(companyIdentityConflictMessage(identityConflicts));
      }
...
export async function publishCompany(id: string): Promise<ActionResult> {
  try {
    ...
    await prisma.$transaction(async (tx) => {
      const identityConflicts = await queryCompanyIdentityConflicts(tx, company.name, id);
      if (identityConflicts.length > 0) {
        throw new AdminActionUserError(companyIdentityConflictMessage(identityConflicts));
      }
...
export async function submitCompanyForReview(id: string): Promise<ActionResult> {
  try {
    ...
    await prisma.$transaction(async (tx) => {
      const updated = await tx.company.updateMany({
        where: { id, status: "DRAFT", ...MUTABLE_COMPANY_WHERE },
        data: { status: "IN_REVIEW" },
      });
      if (updated.count !== 1) {
...
export async function archiveCompany(id: string): Promise<ActionResult> {
  try {
    ...
    await prisma.$transaction(async (tx) => {
      const updated = await tx.company.updateMany({
        where: { id, status: company.status, ...MUTABLE_COMPANY_WHERE },
        data: { status: "ARCHIVED" },
      });
      if (updated.count !== 1) {
...
async function prepareCompanyForOwnershipMutation(
  ...
  const company = await tx.company.findFirst({
    where: { id: companyId, ...MUTABLE_COMPANY_WHERE },
