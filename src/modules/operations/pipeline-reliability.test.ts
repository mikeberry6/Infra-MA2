...
describe("pipeline refresh-window reliability", () => {
  ...
  it("flags an expected critical source that creates no row in two completed windows", () => {
    const rows = [
      sourceRun("SUCCESS", "2026-07-21T11:30:00.000Z", "2026-07-21"),
      sourceRun("SUCCESS", "2026-07-22T11:30:00.000Z", "2026-07-22"),
    ];

    expect(findConsecutiveCriticalSourceIssues(rows, {
      expectedSources: [
        { id: "treasury", name: "U.S. Treasury" },
        { id: "eia", name: "U.S. Energy Information Administration" },
      ],
      refreshWindows: ["2026-07-21", "2026-07-22"],
    })).toEqual(["U.S. Energy Information Administration"]);
  });

  it("waits for two completed windows before declaring an expected source consecutive", () => {
    expect(findConsecutiveCriticalSourceIssues([], {
      expectedSources: [{ id: "eia", name: "U.S. Energy Information Administration" }],
      refreshWindows: ["2026-07-22"],
    })).toEqual([]);
  });
