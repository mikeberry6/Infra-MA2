...
describe("signed authorization snapshots", () => {
  ...
  it("accepts a complete and current snapshot", () => {
  ...
  it("rejects legacy, expired, and implausibly future snapshots", () => {
    ...
    expect(hasUsableSignedAuthSnapshot({
      ...snapshot,
      authenticatedAt: now + 5 * 60 * 1_000 + 1,
    }, now)).toBe(false);
  });
  ...
  it("rejects role, identity, account-version, and deletion mismatches", () => {
    ...
    expect(signedSnapshotMatchesCurrentUser(
      snapshot,
      { ...user, updatedAt: new Date(snapshot.authVersion + 1) },
      now,
    )).toBe(false);
