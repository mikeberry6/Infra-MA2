...
describe("authentication role guards", () => {
  ...
  it("allows administrators and returns their current identity", async () => {
  ...
  it("keeps exports limited to ADMIN and ANALYST", async () => {
    ...
    mocks.getServerSession.mockResolvedValue(session("viewer-1", "PUBLIC"));
    mocks.findUser.mockResolvedValue(user("viewer-1", "PUBLIC"));
  ...
  it("revokes stale privileges after a role, password, or account change", async () => {
    mocks.getServerSession.mockResolvedValue(
      session("admin-1", "ADMIN", 1_700_000_000_000),
    );

    mocks.findUser.mockResolvedValue(
      user("admin-1", "ANALYST", 1_700_000_001_000),
    );
    await expect(requireAdmin()).rejects.toBeInstanceOf(AuthorizationError);

    mocks.findUser.mockResolvedValue(
      user("admin-1", "ADMIN", 1_700_000_001_000),
    );
    await expect(getSessionIdentity()).resolves.toBeNull();
  ...
  it("rejects a valid-looking snapshot after its absolute lifetime", async () => {
    mocks.getServerSession.mockResolvedValue({
      user: {
        id: "admin-1",
        role: "ADMIN",
