...
const IMPORT_SERIALIZATION_ATTEMPTS = 3;
...
export async function commitImport<T>(
  ...
  try {
    for (let attempt = 1; attempt <= IMPORT_SERIALIZATION_ATTEMPTS; attempt += 1) {
      try {
        ...
        if (prismaErrorCode(error) !== "P2034" || attempt === IMPORT_SERIALIZATION_ATTEMPTS) {
          throw error;
        }
        await new Promise((resolve) => setTimeout(resolve, attempt * 50));
      }
    }
    throw new Error("Import serialization retry loop ended unexpectedly");
  } catch (error) {
