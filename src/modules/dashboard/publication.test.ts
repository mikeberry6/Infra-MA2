...
describe("dashboard signal publication gate", () => {
  ...
  it("excludes pending signals from both public sections and risk scoring", () => {
    ...
    expect(approvedView.scorecard.score).toBe(pendingView.scorecard.score);
  });
  ...
  it("excludes approved sample signals from both sections and scoring", () => {
    ...
    expect(sampleView.scorecard.score).toBe(safeView.scorecard.score);
  });

  it("scores only approved signals that are explicitly classified as restrictive", () => {
  ...
  it("does not treat unreviewed keyword-screen counts as supportive demand", () => {
