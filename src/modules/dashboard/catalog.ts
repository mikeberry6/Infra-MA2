...
export const DASHBOARD_SOURCES = {
  ...
  fred: {
    ...
    owner: "Research",
    observationPublicationMode: "AUTOMATIC",
    notes: "Required FRED API adapter for SOFR, spreads, volatility, public-market, macro, and construction series.",
  },
  eia: {
    id: "eia",
    name: "EIA",
    ...
    expectedLagHours: 168,
    termsUrl: "https://www.eia.gov/about/copyrights_reuse.php",
    owner: "Research",
    observationPublicationMode: "AUTOMATIC",
    notes: "Energy price, storage, generation, and grid-data adapter.",
  },
  federalRegister: {
    id: "federal-register",
  ...
  sec: {
    ...
    cadence: "As filed",
    requiresKey: "SEC_USER_AGENT",
    expectedLagHours: 12,
    termsUrl: "https://www.sec.gov/about/privacy-information/security-policy",
    owner: "Research",
    observationPublicationMode: "AUTOMATIC",
    signalPublicationMode: "REVIEW",
    notes: "Configured CIK transaction watchlist plus official XBRL company facts for hyperscaler capex.",
  },
  manual: {
    id: "manual-import",
    name: "Manual Import",
    ...
    expectedLagHours: 168,
    owner: "Research",
    observationPublicationMode: "REVIEW",
    signalPublicationMode: "REVIEW",
  ...
  infrasight: {
    ...
    kind: "manual",
    critical: true,
    url: "/Infra-MA2/tracker",
    cadence: "Daily",
    expectedLagHours: 24,
    owner: "Research",
    observationPublicationMode: "AUTOMATIC",
    notes: "Internal published deal database used for reviewed global deal-flow context.",
  },
...
export const DASHBOARD_METRICS: DashboardMetric[] = [
  metric("us_treasury_2y", "2Y U.S. Treasury CMT", "capital-markets", "Rates", "%", "percent", treasury, "Two-year constant-maturity Treasury yield.", 5),
  metric("us_treasury_5y", "5Y U.S. Treasury CMT", "capital-markets", "Rates", "%", "percent", treasury, "Five-year constant-maturity Treasury yield.", 5),
  metric("us_treasury_10y", "10Y U.S. Treasury CMT", "capital-markets", "Rates", "%", "percent", treasury, "Ten-year constant-maturity Treasury yield.", 5),
  metric("us_treasury_30y", "30Y U.S. Treasury CMT", "capital-markets", "Rates", "%", "percent", treasury, "Thirty-year constant-maturity Treasury yield.", 5),
  metric("curve_2s10s", "2s10s Curve", "capital-markets", "Rates", "bp", "basis-points", treasury, "10Y CMT minus 2Y CMT.", 5),
  metric("curve_5s30s", "5s30s Curve", "capital-markets", "Rates", "bp", "basis-points", treasury, "30Y CMT minus 5Y CMT.", 5),
  metric("tips_10y_real_yield", "10Y TIPS Real Yield", "capital-markets", "Rates", "%", "percent", treasury, "Ten-year Treasury real yield.", 5),
  metric("breakeven_10y_inflation", "10Y Breakeven Inflation", "capital-markets", "Rates", "%", "percent", treasury, "Nominal 10Y CMT minus 10Y TIPS real yield.", 5),
  metric("sofr", "SOFR", "capital-markets", "Funding", "%", "percent", fred, "Secured Overnight Financing Rate.", 5),
  metric("sofr_30d_avg", "30D SOFR Avg.", "capital-markets", "Funding", "%", "percent", fred, "Published 30-calendar-day average SOFR.", 5),
  metric("sofr_90d_avg", "90D SOFR Avg.", "capital-markets", "Funding", "%", "percent", fred, "Published 90-calendar-day average SOFR.", 5),
  metric("sofr_180d_avg", "180D SOFR Avg.", "capital-markets", "Funding", "%", "percent", fred, "Published 180-calendar-day average SOFR.", 7),
  ...
  metric("eia_grid_load", "EIA Lower 48 Grid Load", "sector-micro", "Power / Renewables / Storage", "MWh", "number", eia, "Daily Lower 48 electricity demand aggregated from EIA-930 hourly observations.", 3),
  metric("eia_generation_mix", "EIA Lower 48 Net Generation", "sector-micro", "Power / Renewables / Storage", "MWh", "number", eia, "Daily Lower 48 net generation aggregated from EIA-930 hourly observations.", 3),
  metric("eia_interchange", "EIA Lower 48 Net Interchange", "sector-micro", "Power / Renewables / Storage", "MWh", "number", eia, "Daily Lower 48 total interchange aggregated from EIA-930 hourly observations.", 3),
  ...
  metric("henry_hub", "Henry Hub", "sector-micro", "Power / Renewables / Storage", "$/MMBtu", "currency", fred, "Henry Hub natural gas spot price.", 10),
  ...
  metric("wti", "WTI", "sector-micro", "Power / Renewables / Storage", "$/bbl", "currency", fred, "WTI crude oil spot price.", 10),
  metric("brent", "Brent", "sector-micro", "Power / Renewables / Storage", "$/bbl", "currency", fred, "Brent crude oil spot price.", 10),
  metric("refined_products", "Motor Gasoline Inventories", "sector-micro", "Power / Renewables / Storage", "Mbbl", "number", eia, "Weekly U.S. ending stocks of total motor gasoline.", 14),
  metric("crude_inventories", "Crude Inventories ex-SPR", "sector-micro", "Power / Renewables / Storage", "Mbbl", "number", eia, "Weekly U.S. ending stocks of crude oil excluding the Strategic Petroleum Reserve.", 14),
  ...
  metric("hyperscaler_capex_backlog", "Hyperscaler Annual Capex", "sector-micro", "Digital Infrastructure", "$bn", "currency", sec, "Aggregate latest reported fiscal-year capital expenditures for Microsoft, Alphabet, Amazon, Meta, and Oracle from SEC XBRL company facts.", 400),
...
function metric(
  ...
  return {
    ...
    description,
    staleAfterDays,
    status: ACTIVE_DASHBOARD_METRIC_IDS.has(id) ? "ACTIVE" : "ROADMAP",
  };
}
