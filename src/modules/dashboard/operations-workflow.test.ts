...
describe("dashboard operational workflows", () => {
  ...
  it("keeps writes fail-closed and retains retry, freshness, and source-audit gates", () => {
    ...
    expect(pipeline).toContain("if: vars.PHASE2_PIPELINES_ENABLED == 'true'");
    ...
    expect(schemaStage).toContain("PHASE2_PIPELINES_ENABLED: ${{ vars.PHASE2_PIPELINES_ENABLED }}");
    expect(schemaStage).toContain(
      "FORBIDDEN_DATABASE_HOST: ${{ vars.PHASE2_MIGRATION_DATABASE_HOST }}",
    );
    expect(schemaStage).toContain(
    ...
    expect(schemaStage).not.toContain("vars.MIGRATION_DATABASE_HOST");
    expect(schemaStage).toContain('if [ "$DASHBOARD_WRITES_ENABLED" != "false" ]');
    expect(schemaStage).toContain('if [ "$PHASE2_PIPELINES_ENABLED" != "false" ]');
    expect(schemaStage.indexOf('if [ "$DASHBOARD_WRITES_ENABLED" != "false" ]'))
      .toBeLessThan(schemaStage.indexOf("npm ci"));
    expect(release).toContain('if [ "$PHASE2_PIPELINES_ENABLED" != "true" ]');
  });
