...
export function MobileFilterSheet({
  ...
  useEffect(() => {
    if (!open) return;
    const overlay = panelRef.current?.closest<HTMLElement>("[data-mobile-filter-overlay]");
      ...
      {open && createPortal((
        ...
        >
          ...
          <div
            ...
            tabIndex={-1}
            className="absolute inset-x-0 bottom-0 max-h-[82dvh] overflow-y-auto rounded-t-2xl border-t border-[var(--border)] bg-[var(--bg-app)] shadow-2xl"
          >
            ...
            <div className="sticky bottom-0 border-t border-[var(--border)] bg-[var(--bg-app)]/95 px-4 pb-[max(0.75rem,env(safe-area-inset-bottom))] pt-3 backdrop-blur-md">
              <button
                type="button"
                onClick={() => setOpen(false)}
