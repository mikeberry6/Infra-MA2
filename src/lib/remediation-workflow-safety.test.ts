...
describe("production remediation workflow safety", () => {
  ...
  it("fails closed on target selection and rechecks release provenance before apply", () => {
    ...
    expect(workflow).toContain('forbidden_host_2="$PRODUCTION_POOLED_DATABASE_HOST"');
    expect(workflow).toContain(
      "for value_name in selected_url selected_host selected_name forbidden_host forbidden_host_2; do",
    );
  });

  it("uses the same isolated Phase 2 target as migration validation", () => {
    expect(workflow).toContain(
      "VALIDATION_DATABASE_URL: ${{ secrets.PHASE2_MIGRATION_DATABASE_URL }}",
    );
    expect(workflow).toContain(
      "VALIDATION_DATABASE_HOST: ${{ vars.PHASE2_MIGRATION_DATABASE_HOST }}",
    );
    expect(workflow).toContain(
      "VALIDATION_DATABASE_NAME: ${{ vars.PHASE2_MIGRATION_DATABASE_NAME }}",
    );
    expect(workflow).not.toContain("secrets.MIGRATION_DATABASE_URL");
    expect(workflow).not.toContain("vars.MIGRATION_DATABASE_HOST");
    expect(workflow).not.toContain("vars.MIGRATION_DATABASE_NAME");
  });
