...
async function main() {
  ...
  const [
    ...
    prisma.alias.count(),
    prisma.fund.count(),
    prisma.company.count({ where: ACTIVE_COMPANY_WHERE }),
  ...
  const [
    ...
    prisma.company.count({
      where: { ...ACTIVE_COMPANY_WHERE, ownershipPeriods: { some: {} } },
    }),
    prisma.company.count({
      where: { ...ACTIVE_COMPANY_WHERE, milestones: { some: {} } },
    }),
    ...
    prisma.$queryRaw<Array<{ count: bigint }>>`
      ...
      FROM (
        ...
        WHERE NOT EXISTS (
          SELECT 1
          FROM "CompanyRedirect" r
          WHERE r."retiredId" = c.id
        )
  ...
  const [
    ...
    prisma.ownershipPeriod.findMany({
      where: {
        company: { status: "PUBLISHED", ...ACTIVE_COMPANY_WHERE },
      },
      select: {
        id: true,
        companyId: true,
